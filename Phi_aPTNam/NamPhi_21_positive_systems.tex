...
\begin{remark}
Remark 3.6: We stress out that in previous results on positivity of
autonomous descriptor systems (the case when ..... ) it is assumed
that .... , which is an unnecessary condition, see for instance [11],
[14]. In contrast, our result in Theorem 3.5 provides necessary and sufficient conditions for the positivity of (1) without any a priori assumptions on the projector .... .
\end{remark}


In light of Remark 3.6, we illustrate how Theorem 3.5 applies to
general situations by presenting an example where ...  is not positive
and ....  is not Metzler, but the system is nevertheless positive.
...
\]
and hence the system is not impulse-free, since $\rank(E)=2$. Nevertheless, 
Theorem \ref{thm check advancedness} implies that the system is non-advanced.
Furthermore, by verifying Theorem \ref{Thm positivity} we see that the system is both positive and stable. 
\end{example}
