...
The short outline of this work is as follows. Firstly, in Section \ref{sec2}, we briefly recall the solvability analysis to system \eqref{delay-descriptor}, which is followed by an imporant result about solution comparison for system \eqref{zero-input system} (Theorem \ref{solution comparison 1}). 
...
In this section we discuss the solvability analysis, including the solution representation and the comparison principal for the corresponding IVP to system \eqref{delay-descriptor}, which reads in details
%
\begin{align}
...
Inherited from the theory of delay-different equations (\citet{HalL93}), we recall the concept of \emph{non-advancedness} as follow.
...
the function $x_{j}$ is at least as smooth as $x_{j-1}$ for all $j\in \N$. In deed, most of systems that we have encountered in applications are non-advanced, \citet{AscP95,ShaG06,Ha15}. For more detailed discussions about the types of the DDAE \eqref{zero-input system}, we refer the readers to \cite{Ha15,HaM16,Ung18}.
%
...
\end{equation}
%
It is well-known that in this example $\ind(E,A)=n$. Furthermore, depending on the value of $\ep_2$, the system will be advanced (if $\ep_2\not=0$) and be non-advanced (if $\ep_2\not=0$).
\end{example}
...
%
\end{lemma}
We notice that the matrix products $\hE^D \hE$, $\hE^D \hA$, $\hE\hA^D$, $\hE^D \hB$, $\hA^D \hB$ do not depend on the choice of $\lb$ (see e.g. \citet{Dai89}). Furthermore, they can be numerically computed by transforming the pair $(E,A)$ to their Weierstrass canonical form \eqref{KW form} (see e.g. \citet{Ger05,Vir08}).
...
By simple induction, making use of Lemma \ref{lem2.1},we obtain the solution comparison for system \eqref{delay-descriptor}.

\begin{theorem}\label{solution comparison 1}
Consider system \eqref{delay-descriptor} and assume that it is positive. Let $x(t)$ (resp. $\tx(t)$) be a state function corresponds to a reference input $u(t)$ (resp. $\tu(t)$)
...
\begin{lemma}\label{Castelan'93}
	...
	\[
	[Fz(0) \geq 0] \Rightarrow [Fz(t) \geq 0 \forall t\geq 0]
	\]
...
\begin{proposition}\citet{RamN12}\label{Rami12}
	...
	1) Our descriptor system is positive. \\
	2) There exists a Metzler matrix $H$ s.t. $\bar{A} = H P$, where $\bar{A}:=\hE^D \hA$, $P:=\hE^D \hE$. \\
	3) There exists a matrix $D$ such that $H:=\bar{A} + D \ (I-P)$ is Metzler.
\end{proposition}
...
\begin{lemma}\label{zero state lemma}
...
1) $\hE^D\hA = H \ \hE^D\hE$ for some Metzler matrix $H$. \\
...
\end{theorem}
\begin{proof}
	$\Rightarrow$ We only need to prove part 3. \\
	$\Leftarrow$ Quite simple. 
\end{proof}



If we restrict ourself to the non-delayed case (i.e. $A_d=0$), the direct corollary of Theorem \ref{Thm positivity} is straightforward. We, moreover, notice that this corollary has 
slightly improved the result \cite[Thm. 3.4]{Vir08}.
...
\begin{corollary}\label{Thm positivity - DAE version}
	...
	1) $\hE^D\hA + \a \hE^D\hE \geq 0$ for some $\a \geq 0$. \\
	2) $\hE^D\hB \geq 0$, \\
	3) $C$ is non-negative on the subspace \ $\im_+ \!\left[ \hE^D\hE, (\hE^D\hE - I) \Ka_{\nu}(\hE\hA^D,\hA^D\hB) \right]$.
\end{corollary}
