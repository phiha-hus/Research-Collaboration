%%=======================================================================================
\section{Proof of Lemma \ref{lem3.1}}\label{appendix 1}
%%=======================================================================================
...
\underbrace{ \m{ S^{(2)} B_{11}		& 0  		& 0 \\
		...
		0     		& 0		    & \Si_{0}        \\ 
		0     		& 0         & 0} }_{\tB} \! v(n) \ .  
\quad \pm{ d_{2} \\ s_2 \\ d_1 \\ s_1 \\ r_0 \\ \hline \vphi_2 \\ \vphi_{1} \\ v} 
\ee
%
...
%%=======================================================================================
If this is the case, then it is well-known that one can make use of Kronecker-Weierstra\ss \ canonical form and then to deduce the explicit solution to \eqref{SiDE 1st order}, see e.g. \cite{Dai89}.
\begin{proposition}\label{Kronecker}
	Consider the first order descriptor system \eqref{SiDE 1st order} and assume that $(E,A)$ is a regular pair. Then there exist nonsingular matrices $U$, $V$ such that
	%
	\begin{equation}
	UEV = \m{I_{\td_1} & 0 \\ 0 & N}, \ UAV = \m{J & 0 \\ 0 & I_{\td_2}}, \ \m{B_{11} \\ B_{12}} = UB_1 \ ,
	\end{equation}
	%
	where $N$ is a nilpotent matrix of nilpotency index $\nu(N)$. Consequently, the explicit solution of \eqref{SiDE 1st order} is of the form \  $\xi(n) = V \m{\xi_1(n) \\ \xi_2(n)}$, where
	%
	\be\label{solution}
	\begin{split}
...
\]
%
The set $\mathcal{R}$ of \emph{reachable states} or \emph{reachable set} of \eqref{SiDE 1st order} is the set of all vector that can be reached from some consistent initial vector $\xi(n_0)$ and some input sequence $\{u(n)\}_{n\geq n_0}$. In fact, for \eqref{SiDE 1st order}, it is well-known (e.g. \cite{VerLK81}) that 
%
\[
...
\begin{lemma}\label{strangeness-free vs. impulse free}
	Consider the descriptor system \eqref{descriptor 2nd order discrete} and assume that it is regular. Then system \eqref{descriptor 2nd order discrete} is causal if the associated SiDE \eqref{SiDE 2nd order} is strangeness-free. The converse of this claim, however, does not hold true. 
\end{lemma}
\begin{proof}
...
\begin{example}\label{Exa1}
	...
	\]
	%
	system \eqref{SiDE 1st order} is causal but neither be I-controllable nor strangeness-free.
\end{example}
