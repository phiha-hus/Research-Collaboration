...
\cite{BreCP96, DuLMT13, KunM06, LCB07, L77}. 
Recently, solvability and stability of SiDEs of second order has been investigaed in \cite{LinNT16,LP21,MehrT15}. However, controllability for these systems has not been reached althought it has been well-studied for both DAEs and SiDEs of first order \cite{BeR13,Dai89, KG14}.
...
The first order descriptor system \eqref{SiDE 1st ord} is called 
	...
	%	\item[ii)] \emph{strongly controllable or S-controllable} if both \textbf{C1} and \textbf{C2} holds. 
		...
		\item[iv)] \emph{normalizable} if if if there exists a feedback $u(k)=Kx(k+1)$ such that its closed-loop system $(E+BK)x(k+1)=Ax(k)$ is a explicit difference equation.
	\end{enumerate}
\end{definition}
...
In control theory, classical design approaches usually require that the system is at least S-controllable (and hence, must be Y-controllable). Nevertheless,  
...
\begin{example}\label{example 4}
	...
	\end{equation}
	%
	Clearly, the structure of the pencil $(M,D)$ implies that this system is not Y-controllable, and via one shift we can transform it to the first order system
	%
	\begin{equation*}
		\m{0 & 1 \\ 1 & 0} x(n+1) + \m{0 & -1 \\ -1 & 0 } x(n) = 0 \ ,
	\end{equation*}
	%
	which can be directly proven that is C-controllable. Thus, C-controllability does not imply Y-controllability. \
	%
...
\begin{corollary}\label{coro3}
	...
	%
%	\begin{enumerate}
%		\item[i)] System \eqref{descriptor 2nd order discrete} is Y-controllable via only displacement feedback if and only if in \eqref{condensed form 1}, we have $v=0$ and the matrix $\m{\tM^T_1 & \tM^T_4 & \tD^T_2 & \tD^T_5 & \tK^T_3}^T$ is of full row rank. 
%		\item[ii)] System \eqref{descriptor 2nd order discrete} is Y-controllable via displacement-velocity feedback if and only if in \eqref{condensed form 1}, $v=0$ and the matrix $\m{\tM^T_1 & \tM^T_4 & \tD^T_2 & \tK^T_3}^T$ is of full row rank. 		
%	\end{enumerate}
	%
\end{corollary}
...
\begin{corollary}\label{coro5}
	...
	%
%	\begin{enumerate}
%		\item[i)] System \eqref{descriptor 2nd order discrete} is Y-controllable via only acceleration feedback if and only if $v=0$ and the matrix $\m{\tM^T_1 & \tD^T_2 & \tK^T_3 & \tD^T_5 & \tK^T_6}^T$ is of full row rank. 
%		\item[ii)] System \eqref{descriptor 2nd order discrete} is Y-controllable via d-v-a feedback if and only if $v=0$ and the matrix $\m{\tM^T_1 & \tD^T_2 & \tK^T_3}^T$ is of full row rank. 
%	\end{enumerate}
\end{corollary}
