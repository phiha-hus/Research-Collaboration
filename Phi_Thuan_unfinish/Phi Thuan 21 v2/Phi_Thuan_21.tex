...
% Version 1.1 Thuan 01.05.21
% Version 1.2 Phi  07.06.21
...
\begin{document}
\title{Controllability of second order discrete-time descriptor systems \\ Version \today}
...
In classical approach \cite{Aga00,Ela13,Kel01,MoyKA19,MKA19}, usually new variables are introduced such that a high order system can be reformulated as a first order one. 
...
%
\[
\mathcal{R} = \r^{\td_1} \oplus \im \cK(N,B_{12}),
\]
%
where $\cK(N,B_{12}) := \left[ B_{12}, \ NB_{12}, \ \dots, \ N^{\nu-1}B_{12} \right]$. 
%
%\[
%\cK(N,B_{12}) = \im \left[ B_{12}, \ NB_{12}, \ \dots, \ N^{\nu-1}B_{12} \right] \ .
%\]
%
In particular, if $N=0$, the following corollary is directly followed. 
%
\begin{corollary}\label{coro4}
Assume that the first order, discrete-time descriptor system of the form 
%
\begin{equation*}
\m{\mathbf{E}_1 \\ 0} \xi(n+1) - \m{\mathbf{A}_1 \\ \mathbf{A}_2} \xi(n) = \m{\mathbf{B}_1 \\ \mathbf{B}_2} u(n) \ \mbox{ for all } n \geq 0,
\end{equation*}
%
where $\sm{\mathbf{E}_1 \\ \mathbf{A}_2}$ is nonsingular, and $\mathbf{B}_2$ has full row rank. Then the reachable subspace $\cal{R}$ is the whole space $\r^d$. 
\end{corollary}

\begin{definition}
The first order descriptor system \eqref{SiDE 1st ord} is called 
	\begin{enumerate}
		\item[i)] \emph{completely controllable or C-controllable} if for any $x_0 \in \r^n$ and any $x^f_0 \in \r^n$ there exist a finite time $n_f$ and an input sequence $u$ such that $x(n_f) = x^f_0$.
	%	\item[ii)] \emph{strongly controllable or S-controllable} if both \textbf{C1} and \textbf{C2} holds. 
		\item[ii)] \emph{controllable on a reachable set or R-controllable} if for any $x_0 \in \r^n$ and any $x^f_0 \in \r^n$ there exist a finite time $n_f$ and an input sequence $u$ such that $x(n_f) = x^f_0$.
		\item[iii)] \emph{causal controllable or Y-controllable} if if there exists a feedback $u(k)=Fx(k)$ such that its closed-loop system $Ex(k+1)=(A+B_1F)x(k)$ is causal.
		\item[iv)] \emph{normalizable} if there exists a feedback $u(k)=Fx(k+1)$ such that its closed-loop system $(E+B_1F)x(k+1)=Ax(k)$ is an explicit difference equation, i.e., $E+B_1F$ is nonsingular.
	\end{enumerate}
\end{definition}
...
\begin{proposition}\label{Prop2.3}
	...
	\begin{enumerate}
		\item[i)] C-controllable if and only if \textbf{C0} holds.
	%	\item[ii)] \emph{strongly controllable or S-controllable} if and only if both \textbf{C1} and \textbf{C2} holds. 
		\item[ii)] R-controllable if and only if \textbf{C1} holds. 
		\item[iii)] Y-controllable if and only if \textbf{C2} holds. 
		\item[iv)] normalizable if and only if \textbf{C3} holds. 
	\end{enumerate}
\end{proposition}
%
...
\begin{definition}(\cite{LinNT16}) System \eqref{SiDE 2nd ord} is called \emph{strangeness-free} if there exists a nonsingular matrix $P\in\r^{n,n}$ such that by scaling \eqref{SiDE 2nd ord} with $P$, we obtain a new system of the form
	%
	\be\label{SiDE 2nd order sfree}
	...
	Notice that, restricted to the case that $M=0$, we obtain exactly the well-known concept \emph{strangeness-free} for the first order DAEs in \cite{KunM06}. % In this case, the first block row in \eqref{SiDE 2nd order sfree} is disappeared (i.e., $\hr_{2}=0$).
\end{definition}
...
In control theory, classical design approaches usually require that the system is at least Y-controllable. Nevertheless, this is not always fulfilled as shown in Example \ref{Exa2} below.
...
%
\begin{lemma}\label{lemma LP21} (\cite[Lemma 4.1]{LP21})
...
Moreover, in comparison to \cite{LosM08}, the main advantage of our method is two folds. First, the condensed form is much more concise, and can be computed in a stable way. Second, it is helpful to design a suitable feedback that make the closed-loop system to be causal (resp., impulse-free) in the discrete (resp., continuous) time case.
%
...
% 
\begin{definition}
	The  descriptor system \eqref{descriptor 2nd order discrete} is called \emph{Y-controllable via displac\-ement-velocity-acceleration feedback} if there exists a feedback of the form \eqref{feedback} such that the closed-loop system 
...
\end{theorem}
\begin{proof}
	The proof is followed directly from Lemma \ref{lemma LP21} by consecutively partitioning two matrices $\chD_5$ and $\chD_4$ in \eqref{eq1.6} via Singular Value Decompositions.
\end{proof}
...
Now we are ready to present our first main result about the Y-controllability of \eqref{descriptor 2nd order discrete} in Theorem \ref{Thm 1} below. 
We emphasize, that due to different roles of feedback types, the characteristic condition for Y-controllability via displacement feedback is more strict than
the corresponding one for velocity feedback.
%
\begin{theorem}\label{Thm 1}
	...
	ii) System \eqref{descriptor 2nd order discrete} is Y-controllable via displacement feedback if and only if $v = 0$ and the matrix $\m{M^T_1 & D^T_2 & K^T_3 & D^T_4}^T$ has full row rank. \\
	iii) System \eqref{descriptor 2nd order discrete} is Y-controllable via velocity feedback if and only if $v = 0$ and the matrix $\m{M^T_1 & D^T_2 & K^T_3}^T$ has full row rank. 
\end{theorem}
\begin{proof}
	Since the proofs of these three claims are essentially the same, for the sake of brevity we will present only the detailed arguments for part i).\\
	...
	Finally, notice that system \eqref{descriptor 2nd order discrete} is of square size, so it is regular only if $v=0$. This completes the necessity part. \\
	%
...
%
\be\label{system condensed form 1}
\m{M_{1} & D_{1}    & K_{1}     \\
	0    & D_{2}    & K_{2}     \\
	0    & 0          & K_{3}     \\  \hline
	0    & D_{4}    & K_{4}     \\
...
\ee
%
where $u(n) = V v(n)$ for all $n\geq n_0$. \ Let $z(n) := M_1 x(n+1)$ we can then introduce a new variable $\xi(n) = \sm{z(n) \\ x(n)} \in \r^{r_2+d}$ and rewrite system \eqref{system condensed form 1} in the so-called 
...
\ee
%
% together with an output $y(n) = \m{0 & C}\xi(n)$.
%
...
From Theorems \ref{Thm 1}, \ref{Thm 2} above, we see that one can interpret the upper part of system \eqref{system condensed form 1} as a \emph{causal uncontrollable part}, while the lower part is the 
\emph{causal controllable part}.
Furthermore, the key point for constructing a suitable first order reformulation to \eqref{descriptor 2nd order discrete} (and also for feedback design strategies) is to bring system \eqref{descriptor 2nd order discrete} to the form \eqref{condensed form 1}, where the upper part must be \emph{strangeness-free}, i.e., $\m{M_1^T & D_2^T & K_3^T}^T$ has full row rank. 
In other words, the index reduction procedure has been performed only for the casual uncontrollable part. 
%
Recently, this task has been finished in both theoretical and numerical ways. To keep the brevity of this paper, we will omit the details and refer the interested readers to \cite[Section 4]{LP21}. Below we recall one important result taken from this research.
\end{remark}
...
\begin{proposition}\label{Thm 2.1}(\cite[Theorem 4.7]{LP21})
	...
	$u(n) = V v(n)$ for all $n \geq n_0$, where $V$ is nonsingular. Furthermore, if system \eqref{descriptor 2nd order discrete} is regular then \ $\hv=0$.
\end{proposition}
...
The following corollaries are direct consequences of Theorem \ref{Condensed form 2} and Lemma \ref{lemma LP21}.
...
Consider the descriptor system \eqref{descriptor 2nd order discrete} and the factorization \eqref{condensed form 2}. Then, the following assertions hold true.\\
	i) System \eqref{descriptor 2nd order discrete} is Y-controllable via only displacement feedback if and only if in \eqref{condensed form 1}, we have $v=0$ and the matrix 
	$\m{\tM^T_1 & \tD^T_2 & \tK^T_3 & \tM^T_4 & \tD^T_5 }^T$ is of full row rank. \\
	ii) System \eqref{descriptor 2nd order discrete} is Y-controllable via displacement-velocity feedback (or velocity feedback) if and only if in \eqref{condensed form 1}, $v=0$ and the matrix 
	$\m{\tM^T_1 & \tD^T_2 & \tK^T_3 & \tM^T_4 }^T$ is of full row rank. 		
\end{corollary}

\begin{corollary}\label{coro5}
Consider the descriptor system \eqref{descriptor 2nd order discrete} and the factorization \eqref{condensed form 2}. Then, for any kind of feedback that involves acceleration (d-v-a, d-a, v-a, a),
system \eqref{descriptor 2nd order discrete} is Y-controllable via that feedback type if and only if $v=0$ and the matrix $\m{\tM^T_1 & \tD^T_2 & \tK^T_3}^T$ is of full row rank. 
\end{corollary}
...
\begin{example}
	Similarly, using Corollaries \ref{coro3}, \ref{coro5} we see that one could not use only displacement-velocity feedback to eliminate the causal behavior of the system
	%
	\begin{equation*}
	...
	\end{equation*}
	%
	We notice, that we can construct any of four feedback types (d-v-a, d-a, v-a, a) to regularize this system.
\end{example}

\begin{remark} We also notice, that even though different feedback types can be applied to achieve the causality of the closed-loop systems, two condensed forms \eqref{condensed form 1} and 
	\eqref{condensed form 2} are still useful to achieve a desired rank for the system, i.e., there is a desired number of zero-, first- and second-order equations. For more details on this 
	issue, we refer the readers to \cite{BunBMN99,ChuM00,ChuMN99}.
\end{remark}
%=================================================================================================================
\section{Other controllability concepts and their characterizations}\label{Sec4}
%=================================================================================================================

In this section, using the condensed forms \eqref{condensed form 1}, \eqref{descriptor minimal extension} proposed above, we will discuss other controllability concepts for second order systems. We will also point out the difference between a discrete and continuous time cases and discuss a new feature of second order system as well. 
...
It is straightforward to see that all these controllability concepts are invariant under left equivalent transformation. In the following theorem, we give a characterization for the strongly C2- and R2-controllability.

\begin{theorem}\label{Thm strongly C2 & R2}
...
\end{theorem}
\begin{proof}
Following directly from Definition \ref{C2 concepts}, we see that system \eqref{descriptor 2nd order discrete} is  strongly C2-controllable (resp., R2-controllable) if and only if 
its first order companion form \eqref{first order companion form} is C-controllable (resp., R-controllable). Thus, the proof is directly followed by checking the rank criteria in  
...
\begin{lemma}\label{lem4.5} Consider the descriptor system \eqref{descriptor 2nd order discrete} and its the strangeness-free from \eqref{descriptor 2nd order sfree} and the 
	...
	iii) The constant rank condition $\mathbf{C21}$ is preserved under the strangeness-free formulation, which transform \eqref{descriptor 2nd order discrete} to \eqref{descriptor 2nd order sfree}.
\end{lemma}
\begin{proof}
...
Due to Definition \ref{Prop2.3}, system \eqref{descriptor minimal extension} is $R$-controllable (resp. $C$-controllable) if and only if the matrix coefficients $\tE$, $\tA$, $\tB$ satisfy the constant rank $\mathbf{C1}$ (resp., $\mathbf{C0}$). \\
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
ii) Due to Definition \ref{Prop2.3}, we see that $\mathbf{C0}=\mathbf{C1}+\mathbf{C3}$, and hence we need to prove that condition $\mathbf{C3}$ is equivalent to condition $\mathbf{C23}$. 
...
\begin{theorem}\label{thm4}
	Consider the descriptor system \eqref{descriptor 2nd order discrete}, its the strangeness-free from \eqref{descriptor 2nd order sfree} and the minimal extension form \eqref{descriptor minimal extension}. 
	If system \eqref{descriptor 2nd order discrete} is R2-controllable then so is system \eqref{descriptor 2nd order sfree}. Furthermore, if this is the case, then system \eqref{descriptor minimal extension} is $R$-controllable.
\end{theorem}
\begin{proof} Making use of Theorem \ref{Thm strongly C2 & R2} i) and Lemma \ref{lem4.5} ii) we see that the constant rank condition $\mathbf{C21}$ holds for the coefficients of system \eqref{descriptor minimal extension}.
...
\begin{example}\label{exam3.2}
	...
	\]
	%
	Analogously, by subtracting the shifted version of the first row equation from the second equation, we obtain the strangeness-free formulation \eqref{SiDE 2nd order sfree} that reads
	%
	\be\label{eq4.2}
...
\begin{proof} 
	...
	From Definition \ref{Prop2.3} and Lemma \ref{lem4.5} iii, we see that $\mathbf{C0}=\mathbf{C1}+\mathbf{C3}$ and $\mathbf{C1}$ is equivalent to condition $\mathbf{C21}$. \\ 
...
\end{theorem}
\begin{proof}
	Notice that in system \eqref{descriptor minimal extension} $\xi_n = \sm{z_n \\ x_n} \in \r^{r_2+d}$, so the equivalence between i) and ii) is straightforward. 
	...
	\]
	%
also has full row rank, and hence, system \eqref{eq4.3} is regular and strangeness-free. 
Corollary \ref{coro4}
...
By following \cite{Dai89}, we can determine the reachable set $\cR$ of system \eqref{eq4.3} based on the Kronecker-Weierstra\ss canonical form of \eqref{descriptor 2nd order discrete}
%
...
for the corresponding first order system \eqref{eq4.4} the matrix product $\m{0 & I_{n_1-r_2}} \cK(\barA_1 ,\barB_1)$ has full row rank, where
%
\begin{equation}\label{eq4.5}
 \cK(\barA_1 ,\barB_1) := \m{ \barB_1, \ \barA_1 \barB_1, \ \dots, \ \barA_1 ^{n_1-1}\barB_1 }, 
\end{equation}
% 
Here the matrix $\m{0 & I_{n_1-r_2}} \in \R^{n_1-r_2,n_1}$.
\end{theorem}
\begin{proof}
...
$\im \cK(\barA_1 ,\barB_1) = \R^{n_1}$. Furthermore, notice that the first $r_2$ variables of \eqref{eq4.3} come from the transformation of second order system \eqref{descriptor 2nd order sfree} to the first order system \eqref{eq4.3} and are not relevant to consider for R-controllability. Therefore, the proof is straightly followed.
\end{proof}
...
\begin{figure}[!h]
	...
	\begin{tikzpicture}[scale=1.0, transform shape]
		...
		\draw[->] (a) -- (b) node[pos=.5,sloped,above] {\textbf{$R2$-ctrb $\Leftrightarrow R2$-ctrb}};
		\draw[->] (b) -- (d) node[pos=.5,sloped,above] {\textbf{$R2$-ctrb $\Leftrightarrow R$-ctrb}};
		\draw[->] (b) -- (d) node[pos=.5,sloped,below] {\textbf{$C2$-ctrb $\Leftrightarrow C$-ctrb}};
		\draw[->] (a) -- (c) node[pos=.5,sloped,above] {\textbf{$R2$-ctrb $\Leftrightarrow R$-ctrb}};
		\draw[->] (a) -- (c) node[pos=.5,sloped,below] {\textbf{strongly $C2$-ctrb $\Leftrightarrow$ $C$-ctrb}};
		\draw[-] (a) -- (d) node[pos=.5,sloped,above] {\textbf{$C2$-ctrb $\overset{\Delta}{\Longleftrightarrow} C$-ctrb}};
		\draw[-] (d) -- (c) node[pos=.5,sloped,above] {\textbf{$C$-ctrb $\rightarrow$ $C$-ctrb}};
		\draw[-] (d) -- (c) node[pos=.5,sloped,below] {\textbf{$C$-ctrb \cancel{$\leftarrow$} $C$-ctrb}};
...
%=================================================================================================================
In this section we give a few result about the corresponding observability of system \eqref{descriptor 2nd order discrete}.
For this let us denote by $\cP_{r,2}$ the projection onto the right finite eigenspace corresponding to the finite eigenvalues of the matrix polynomial $\lb^2M+\lb D+K$, \cite{GohLR82}. 
First we recall three important concepts.

\begin{definition}
i) System \eqref{descriptor 2nd order discrete} is called \emph{$C$-observable} if from a response $y=0$ for the input $u=0$ it follows that system \eqref{descriptor 2nd order discrete} has only one trivial solution $x=0$. \\ 
ii) It is called \emph{$R$-observable} if from a response $y=0$ for the input $u=0$ it follows that $\cP_{r,2}x=0$. \\
iii) It is called \emph{causal observable ($Y$-observable)} if its state $x(k)$ at any time point $k$ is uniquely determined by initial condition $(x(0),x(1))$ and the former ($k$ included) inputs $u(i)$, together with former outputs $y(i)$, $i=0,\dots,k$.  
\end{definition}

\begin{remark}
Due to linear property of system \eqref{descriptor 2nd order discrete}, $C$-observability also means that 
for any unknown initial condition $(x(0),x(1))$, there exists a finite integer $k>0$, such that the knowledge about former ($k$ included) inputs $u(i)$, together with former output $y(i)$, $i=0,\dots,k$  suffices to determine uniquely the initial condition $(x(0),x(1))$.
\end{remark}

It is straightforward to see that all three observability concepts above are invariant under left equivalent transformation. 
On the other hand, since the index reduction procedure, which transforms system \eqref{descriptor 2nd order discrete} to the form \eqref{descriptor 2nd order sfree}, does not alter the solution set of system \eqref{descriptor 2nd order discrete}, the C- and R-observability are preserved.
Furthermore, due to Remark \ref{rem 3.10}, the index reduction procedure has been performed only on the causal uncontrollable part, which implies that the Y-observability is also preserved. 
%
The following lemma plays the key role in our study about the observability of \eqref{descriptor 2nd order discrete}.
%
\begin{lemma}\label{lem5.1}
Consider system \eqref{descriptor 2nd order discrete}, the the strangeness-free from \eqref{descriptor 2nd order sfree} and the minimal extension form \eqref{descriptor minimal extension}. Then, system \eqref{descriptor 2nd order sfree} is Y-observable (resp., $R$-observable) if and only if system \eqref{descriptor minimal extension} is also Y-observable 
(resp., $R$-observable).
\end{lemma}
\begin{proof}
Concerning about the $Y$-observability, the proof is straightforward, since the transformation from \eqref{descriptor 2nd order sfree} to \eqref{descriptor minimal extension} keeps both the input and output, while the second block equation of \eqref{descriptor minimal extension} is nothing else than $z(n)=Mx(n+1)$, which does not have any impact on the causality of the system. \
About the $R$-observability, the proof is essentially the same as the proof of \cite[Thm 4.3]{LosM08}, so we will omit it to keep the brevity of this paper.
\end{proof}

Making use of Lemma \ref{lem5.1}, we see that the first order duality of controllability and observability \cite{Dai89,Dua10} can be directly extended to the second order case 
for system \eqref{descriptor 2nd order discrete} and the dual system 
%
\begin{equation}\label{dual system}
	\begin{split}
		& M^T x(n+2) + D^T x(n+1) + K^T x(n) = C^T u(n) \ \mbox{ for all } n \geq n_0, \\
		&y(k) = Bx(k),\\
		& x(n_0) = x_0, \ x(n_0+1) = x_1 .
	\end{split}
\end{equation}
%
%\textbf{Theorem \ref{Thm Obsv} and Corollary \ref{coro 5.1} are translated from Loose/Mehrmann.}
\begin{theorem}\label{Thm Obsv}
Consider the second order descriptor system \eqref{descriptor 2nd order discrete} and the dual system \eqref{dual system}. Then the following assertions hold true.\\
i) System \eqref{descriptor 2nd order discrete} is $C$-observable if and only if the dual system \eqref{dual system} is $C2$-controllable. \\
ii) System \eqref{descriptor 2nd order discrete} is $R$-observable if and only if the dual system \eqref{dual system} is $R2$-controllable. \\
iii) System \eqref{descriptor 2nd order discrete} is $Y$-observable if and only if the dual system \eqref{dual system} is $Y$-controllable via displacement-velocity feedback.
\end{theorem}
\begin{proof}
Due to Lemma \ref{lem5.1}, the proof is directly obtained by checking rank conditions for the first order system \eqref{descriptor minimal extension}, so it will be omitted to keep the brevity of this paper.
\end{proof}

\begin{corollary}\label{coro 5.1}
Consider the second order descriptor system \eqref{descriptor 2nd order discrete}. Then, it is \\
i) $R$-observable if and only if 
%
\[ \rank \m{\lb^2M+\lb D+K \\ C} = d ; 
\]
%
ii) $C$-observable if and only if it is $R$-observable and the matrix coefficients in the strangeness-free form of the dual system \eqref{dual system} satisfy
%
\begin{equation}\label{eq5.1}
\rank \m{\hM^T & \hD^T & \hC^T} = d ; 
\end{equation}
%
iii) $Y$-observable if and only if 
%
\[
\rank \m{M \\ \ T^1_{\infty} D \\ T^2_{\infty} K \\ C} = d, 
\]
%
where rows of $T^1_{\infty}$ form a basis of $\cokernel M$, and rows of $T^2_{\infty}$ form the basis of 
%
\[
\cokernel \sm{M \\ D Z_1} \setminus \cokernel \sm{ M \\ D Z_1 \\ K Z_3},
\]
%
and rows of $Z_1$ and of $Z_3$ form a basis of $\kernel M$ and $\kernel \m{M \\ D}$, respectively.
\end{corollary}

In analogous to the controllability case, see Example \ref{exam3.2}, here we notice that the rank condition $\rank \m{M^T \ D^T \ C^T} = d$ implies \eqref{eq5.1}, but the converse is not true.
Consequently, system \eqref{descriptor 2nd order discrete} may not be $Y$-observable, even if $\rank \m{M^T \ D^T \ C^T} = d$, as illustrated in the following example.

\begin{example}\label{exa5.1}
Consider the system \eqref{descriptor 2nd order discrete} which reads
%
...
+ \underbrace{\m{1 & 0 & 0 \\ 0 & 1 & 0 \\ 0 & 0 & 0 }}_{K} x(n) 
= \underbrace{\m{0 \\ 0 \\ 1}}_{B} u(n), \\
& y(n) = \underbrace{\m{0 & 0 & 1}}_{C} x(n) \ . \notag 
\end{align}
%
Since the matrices $M$, $D$, $K$ are symmetric and $C^T=B$, we see that the dual system of \eqref{eq5.3} is nothing else than itself.
As in Example \ref{exam3.2}, the strangeness-free formulation of this dual system reads
%
\be\label{eq5.4}
\underbrace{\m{0 & 0 & 0 \\ 0 & 0 & 0 \\ 0 & 0 & 0 }}_{\hM^T} x(n+2) 
+ \underbrace{\m{0 & 0 & 0 \\ 0 & 0 & 0 \\ 0 & 0 & 0 }}_{\hD^T} x(n+1) 
+ \underbrace{\m{1 & 0 & 0 \\ 0 & 1 & 0 \\ 0 & 0 & 0 }}_{\hK^T} x(n) 
= \underbrace{\m{0 \\ 0 \\ 1}}_{\hC^T} u(n) \ .
\ee
%
Consequently, the dual system is not $C$-controllable (and hence not $C2$-controllable). Theorem \ref{Thm Obsv} i) applied to system \eqref{eq5.3} implies that this system is not $C$-observable, 
despite the fact that $\rank \m{M^T \ D^T \ C^T} = 3$. This agrees with Corollary \ref{coro 5.1} ii), since $\rank\m{\hM^T \ \hD^T \ \hC^T}=1<3$. 
Besides that, by direct computation, we see that system \eqref{eq5.3} is $R$-observable but not $Y$-observable.
%from \eqref{eq5.3}, we see that $y(n)=u(n)=0$ for all $n\geq 0$ implies that $x(n)=0$, and hence system \eqref{eq5.3} is $C$-observable.
\end{example}
