...
%
% Loesen der differentiell-algebraischen Gleichung 1. Ordung der Form
% M y'(t)= S y(t) + f(t,y,y')
%
% mit BDF der Ordnung Ordnung k
%
% CALL  : [t,h,y,y_est,y_interpol]=bdfk1_dae(funct,y0,y1,t0,a,N,K,RTOL,ATOL,H0)
%
...
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [t,h,y,y_est,y_interpol]=bdfk1_dae(funct,funct_d1,funct_d2,y0,t0,a,N,RTOL,ATOL,H0,step,maxOrder)
...
% initial stepsize
if(H0==0)
    H0 = sign(TOUT-t0)*min(10^(-3)*abs(TOUT-t0),0.5*myNorm(y0,RTOL,ATOL)^(-1));
end
...
%iNoRej=0;nef=0;ncf=0;nsf=0;iPhase=0;
newOrder = 0;

% integration
i=1;I=1;

while(i<=7000 && t(i)<=TOUT)
    %i
    t(i+1) = t(i)+h(i);
    h(i+1) = h(i);
    ...
    if( i<=4 )
        ...
        if(newOrder==1) % increase order
            ...
            if(newOrder==2) % decrease order
                if( k ~=1 )
                    k = k-1;
                end
            else
                if(newOrder==3) % order constant
                    k=k;
                else
    ...
    [y(:,i+1),converged,IER] = bdf1_step(funct,funct_d1,funct_d2,h,t,i,n,k,y,RTOL,ATOL);
