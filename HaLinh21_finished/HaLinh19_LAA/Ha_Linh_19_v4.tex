...
% Version 4 : 30/03/2020 Phi, 6/4 Linh
...
\begin{proof} The block diagonal form \eqref{block-upper-M} is obtained directly by consecutively compressing the block columns $A_{n}$, $B_{n}$, $C_{n}$ of $M_n$ via Lemma \ref{lem1.4}. In details, we have that 
	...
	\mbox{ rows of } C_{n,3} &\mbox{ form the basis of the space } \range\left(T^T_0 \left(\sm{A_n^T \ B_n^T}^T \right)\,  C_n\right)^T .
	\end{align*}
	%	
...
\rm{
	...
	& \rank(A_{n}) \!\equiv\! \const, \ \rank(\m{A_{n} & B_{n}}) \!\equiv\! \const , \ \rank(\m{A_{n} &  B_{n} & C_{n}}) \!\equiv\! \const, \notag \\
	& \rank(T^T_0(A_n) \, B_n) \!\equiv\! \const, \ \rank \left(T^T_0 \left(\sm{A_n^T \ B_n^T}^T \right) \, C_n\right) \!\equiv\! \const
	\end{align}
}
\end{rmk}
...
\begin{lem}\label{lem1.5} Given four matrices $\chA$, $\chB$, $\chC$ in $\r^{m,d}$ and $\chD$ in $\r^{m,p}$. Let us consider the following matrices whose columns span orthogonal bases of the associated vector spaces
	...
	\begin{enumerate}
		\item[i)] The matrices $\sm{T_{i,\perp}^T \ T_{i}^T }^T$, $i=1,...,4$, and $\sm{W_{1,\perp}^T \ W_1^T}^T$ are orthogonal.
		...
		\ee
		where the matrices $\chA_1$, $\chB_2$, $\chB_4$, $\chC_3$, $\sm{\chD_4^T \ \chD_5^T}^T$ have full row rank.
	\end{enumerate}	
	%		
\end{lem}
...
\end{thm}
\begin{proof}
i)	First we apply Lemma \ref{lem1.5} to four matrices $A_n$, $B_n$, $C_n$ and $D_n$ to obtain the matrix $U_n$ that satisfies \eqref{eq1.6}. Decompose the matrix $\sm{\chD_4^T \ \chD_5^T}^T$ via one SVD, we then obtain the block 
%
$ \sm{
	0      	    & \Si_{n,1} 	    & 0 		 \\ 
	0     		& 0		    & \Si_{n,0} } \ .
...
\begin{lem}\label{lem5.2} Assume that the upper part of the descriptor system \eqref{eq3.3} is not strangeness-free. 
	...
	\end{equation}
	%
	for all $n\geq n_0$. Here, we have $\tr_2=r_2-s_2$, $\tr_1=r_1+s_2-s_1$, $\tr_0 = r_0+s_1$, $\tv\geq v$ for some $s_2>0$, $s_1>0$. Furthermore, both pairs $\left(\tA_{n,1},\sm{\tB_{n,2}^T \ \tC_{n,3}^T}^T \right)$ and $\left(\tB_{n,2},\tC_{n,3} \right)$ have no hidden redundancy.
\end{lem}
\begin{proof}
...
\ee
%
Due to Lemma \ref{lem1.4}, we have that $U_1^T \cW = 0$ and $U_{1,\perp}^T \cW$ has full row rank. Furthermore, the matrix $\sm{U_1 \ U_{1,\perp}}^T$ is nonsingular, and hence, system \eqref{inflated} is equivalent to the coupled system below. 
%
\begin{alignat}{3}
...
\begin{algorithm}[H]
	...
	\begin{algorithmic}[1]
		...
		%				\algrestore{myalg}
		%				% 
		\State Find the matrices $S^{(1)}_{n}$, $S^{(2)}_{n}$ in the process used to remove the hidden redundancies in two matrix pairs $\left(\chB_2, \ \chC_3 \right)$, $\left(\chA_1, \sm{\chB_2 \\ \chC_3 } \right)$, respectively.
...
is under our on-going research. Furthermore, the condensed forms presented in this work also motivate further study on the staircase form for second order systems, which would be an extension of classical results for first order systems, e.g. \cite{Son13}.
...
Analogously, if the pair $\left( A_{n,1}, \sm{B_{n+1,2}^T \ C_{n+2,3}^T}^T \right)$ has hidden redundancy then we will make use of the shifted equation
%
\begin{equation}\label{2.5b-shift}
B_{n+1,2} x(n+2)  + C_{n+1,2} x(n+1) = f_2(n+1),
\end{equation}
