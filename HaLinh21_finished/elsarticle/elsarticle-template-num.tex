...
\title{Index Reduction for Second Order Singular Difference Equations}
...
Singular systems \sep Difference equation \sep Descriptor systems \sep Strangeness-index \sep Regularization \sep Feedback.
...
\end{lem}
%
\begin{pf} First using SVD we factorize $Q_{n}$ and then partition $P_{n}$ conformably to get
	%
...
\begin{rmk} Following directly from the proof of Lemma \ref{lem2.1}, we see that Assumption \ref{Ass1} is satisfied if and only if five following constant rank conditions are satisfied
	%
...
\begin{rmk} In system (\ref{def sfree}), the quantities $r_2$, $r_1$, and $r_0$ are the dimensions of the second-order dynamics part, the first-order dynamics part, and the algebraic (zero-order) part, respectively. Furthermore, $r_2 + r_1$ is exactly the degree of freedoms. 
\end{rmk}
