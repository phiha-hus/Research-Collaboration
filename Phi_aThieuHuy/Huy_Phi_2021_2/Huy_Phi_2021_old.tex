...
%\usepackage{color}
%%\usepackage[inline,right]{showlabels}
%\usepackage[inline]{showlabels}
%\renewcommand{\showlabelfont}{\sriptsize\slshape\color{red}}
%\renewcommand{\showlabelfont}{\ttfamily\small\color{red}} 
...
%\email{haphi.hus@vnu.edu.vn}
%
%\author[N.T. Huy]{Thieu Huy Nguyen}
%\address{Nguyen Thieu Huy, School of Applied Mathematics and Informatics,
%	Hanoi University of Science and Technology,
%	Vien Toan ung dung va Tin hoc, Dai hoc Bach khoa Ha Noi,
%	1 Dai Co Viet, Hanoi, Vietnam}
...
%
\begin{equation}\label{semi linDAE}
\pm{d \mbox{ rows }\\ a \mbox{ rows }} \quad \underbrace{\m{\bfE_1(t) \\ 0}}_{E(t)}  \dot{x}(t) = \underbrace{ \m{\bfA_1(t) \\ \bfA_2(t)} }_{A(t)} x(t) + 
...
%
\begin{equation}\label{linDAE}
E(t)\dot{x}(t) = A(t) x(t), \quad t\in [0,+\infty).
\end{equation}
%
Here $E=\m{\bfE_1(t) \\ 0}$, $A = \m{\bfA_1(t) \\ \bfA_2(t)}$ are matrix-valued functions acting 
on $\r_+$ to $\r^{n,n}$, $x: \r_+  \rightarrow \r^{n}$, and $f:\r_+ \times \r{^n} \rightarrow \r{^n}$. Furthermore, we assume that for all $t$, the matrices $\bfE_1(t)$, $\bfA_2(t)$ have full row rank. 

DAE systems of the forms \eqref{semi linDAE}, \eqref{linDAE} arise in many applications, include multibody dynamics, electrical circuits, chemical engineering, and many other applications. 
...
the spectral theory of DAEs, (in particular, the exponential dichotomy for DAEs), has attracted much attention only recently and is still an active research area.
We refer to \cite{LenM90} for the concept of exponential dichotomy and its relation to the well conditioning of the associated boundary value problem, to \cite{Mas13,Mas15,LinM09} for Lyapunov and other spectra for linear DAEs, to \cite{Ber12,ChyDL08,DuL06} for the robustness of exponential stability and Bohl exponents, 
to \cite{Dia18,DuLN16,ThuNHQ20} for the study of singular difference systems, and to \cite{TroW19} for the exponential dichotomy of infinite dimensional systems. 

On the other hand, whenever the exponential dichotomy of the linear, homogeneous system \eqref{linDAE} is characterized, 
the next important question in the qualitative theory of DAEs is to study the existence of  integral manifolds (e.g., stable, unstable, center, center-stable, center-unstable) for the semi-linear DAE \eqref{semi linDAE} \cite{DalK02,Chi13}. 
Unfortunately, till now this question is essentially open for DAEs. In order to shorten these gaps, this paper is devoted to investigation of
 the exponential dichotomy of \eqref{linDAE} and stable manifolds of \eqref{semi linDAE}. Our method is based on the classical "Lyapunov-Perron method" (\cite{DalK02,Zhi72}) and the admissibility of function spaces (\cite{Huy06,Huy09a}). 
...
%Then, in Section \ref{Sec 2} we present the admissibility of function spaces and their properties. 
 In Section \ref{Sec 3} we gives a characterization for the existence of exponential dichotomy for the DAE \eqref{linDAE}. 
Section \ref{Sec 4} contains our main results on the existence and properties of local stable manifold for the semi-linear DAE \eqref{semi linDAE}. The global version of these results will be presented in Section \ref{Sec 5}. Finally, we illustrate our results by studying a spatial discretization of  Navier-Stokes equations, and we conclude this research by a summary and some open problems.\\
...
\end{equation}
%
Furthermore, every solution of the corresponding semi-linear ODE
%
...
%\ee
%
also satisfies the so-called \emph{variation-of-constant formula}
%
...
\begin{enumerate}
	\item[i)] $P(t)X(t,s)=X(t,s)P(s)$  for all  $t\geq s\geq 0$, 		
	\item[ii)] for all $t\geq s \geq 0$, the restriction $X(t,s)_{|}: \ker P(s) \rar \ker P(t)$ is an isomorphism, and we denote its inverse by $X(s,t)_{\mid}$, 
...
The concept exponential dichotomy means that the state space $\r^n$ has been splitted into the (exponentially) stable subspace ($\im(P(t))$)
and the (exponentially) unstable subspace ($\ker(P(t))$). 

%================================================================================================================================
\subsection{A short review of DAE solvability theory}
%================================================================================================================================
%Now let us recall some basic concepts and properties for DAEs, starting with \emph{fundamental solution matrix} as below.

Linear DAEs of the form \ref{linDAE} have been extensively studied in the last thirty years, see \cite{KunM06} and the references therein. 
In order to understand the solution behavior and to obtain numerical solutions, the necessary information about derivatives of equations has to be utilized. 
This has led to the concept of the strangeness index, which allows to use the DAE and (some of) its derivatives to be reformulated as a system with the same solution that is \emph{strangeness-free}, i.e., for which the algebraic and differential part of the system are easily separated. In this paper we restrict ourselves to regular DAEs with sufficiently smooth coefficients, i.e., we require that \eqref{linDAE} (or the nonlinear DAE \eqref{semi linDAE} locally) has a unique solution for appropriately chosen (consistent) initial conditions, see \cite{KunM06} for a discussion of more general nonregular DAEs. 
With the theory and appropriate numerical methods available, then throughout this paper, for regular DAEs we may assume that the homogeneous DAE \ref{linDAE} in consideration fulfills the following presumption.

%
\begin{assumption}\label{Ass1.0}
Assume that the function pair $(E,A)$ in the DAEs \eqref{semi linDAE}, \eqref{linDAE} is \emph{strangeness-free}, i.e., 
%
\[
\rank \m{\bfE_1(t) \\ \bfA_2(t)} = n,
\]
%
...
\begin{equation}\label{eq6}
	\tE(t) \dot{y}(t) = \tA(t) x(t) + \tf(t,y(t))
\end{equation}%
%
is called \emph{orthogonally equivalent} to the DAE \eqref{semi linDAE} if there exist 
pointwise-orthogonal matrix-valued functions $U\in C^0([0,\infty),\r^{n,n})$ and $V\in C^1([0,\infty),\r^{n,n})$, such that after changing variable $x(t) = V(t)y(t)$, and scaling \eqref{linDAE} with $U(t)$, we obtain exactly \eqref{eq6}. In details, this means that the following identities hold true.
%
\begin{equation*}
\tE = U E V, \ \tA = UAV-UE\dot{V}, \ \tf(t,y(t)) = U(t)f(t,Vy(t)), \mbox{ for all } t\geq 0.
\end{equation*}
%
We denote this orthogonal equivalence by $(E,A,f) \osim (\tE,\tA,\tf)$ and omit the terms $f$, $\tf$ if the homogeneous system \eqref{linDAE} is considered. 
\end{definition}
%
Indeed, one can directly verify that this orthogonal equivalence concept is an equivalent relation, i.e., it fulfills three properties: reflexivity, symmetry and transitivity. We omit the detailed proof here in order to keep the brevity of this work. \
%%
...
%%
By making use of some smooth factorizations, for example QR or SVD (\cite{DieE99}  or \cite[Thm 3.9]{KunM06}), we can decouple the differential and algebraic parts of the DAE \eqref{linDAE} in the following lemma.

%\begin{lemma}\label{lem0}
%	Consider the DAE \eqref{semi linDAE} and assume that it satisfies Assumption \ref{Ass1.0}. \\
%	i) Then, there exists a continuous, pointwise-orthogonal matrix function $V(t)$, $t\geq 0$, such that by changing variable $x(t) = V(t)y(t)$, we can transform \eqref{semi linDAE} to the form
%	%
%	\begin{equation}\label{semi-explicit system}
...
%	ii) Consequently, the DAE \eqref{semi linDAE} becomes the coupled system
%	%
%	\begin{align}\label{coupled system}
%	\label{semi-explicit system.1} \Si(t) \dot{y}_1(t) &=  A_1(t)y_1(t) + A_2(t)y_2(t) + \tf_1(t,y_1(t),y_2(t)), \\
%	\label{semi-explicit system.2} 0 &= A_3(t)y_1(t) + A_4(t)y_4(t) + \tf_2(t,y_1(t),y_2(t)).
%	\end{align}
%	%
%\end{lemma}

\begin{lemma}\label{lem1}
Consider the DAE \eqref{linDAE} and assume that it satisfies Assumption \ref{Ass1.0}. Then, there exist pointwise-orthogonal matrix-valued functions $U\in C^0([0,\infty),\r^{n,n})$ and $V\in C^1([0,\infty),\r^{n,n})$, such that by changing variable $x(t) = V(t)y(t)$, and scaling \eqref{linDAE} with $U(t)$, we obtain the so-called \emph{semi-explicit system}
%
\be\label{semi-explicit system}
...
%
\end{lemma}
\begin{proof} Applying an SVD factorization for $\bfE_1(t)$ we can find 
pointwise-orthogonal matrix functions $U_1(t) \in C^1([0,\infty),\r^{d,d})$ and $V\in C^1([0,\infty),\r^{n,n})$	
such that
%
$ U_1(t) \bfE_1(t) V(t) = \m{\Si(t) & 0}$,
%	  
where $\Si(t)$ is a continuous, pointwise nonsingular function with values in $\r^{d,d}$. Changing the variable $x(t) = V(t)y(t)$ and scaling \eqref{linDAE} with $U(t):=\sm{U_1(t) & 0 \\ 0 & I_{a}}$, we obtain a new system exactly of the form \eqref{semi-explicit system}. Furthermore, notice that 
%
\[
\m{\Si(t) & 0 \\ A_3(t) & A_4(t)} = \m{U_1(t) & 0 \\ 0 & I_{a}} \ \m{\bfE_1(t) \\ \bfA_2(t)} \ V(t),
\]
%
then Assumption \ref{Ass1.0} yields that both $\Si$ and $A_4$ are nonsingular. This completes the proof.
\end{proof}
%
Let $\hA_3 := -A^{-1}_4 A_3$, $\hA_1 := \Si^{-1} \left( A_1 - A_2 A^{-1}_4 A_3 \right)$, we rewrite system \eqref{semi-explicit system} as 
%
\begin{subequations}\label{eq8}
\begin{alignat}{2}
 \dot{y}_1(t) &= \hA_1(t) y_1(t), \label{eq8.1} \\ 
       y_2(t) &= \hA_3(t) y_1(t). \label{eq8.2}
\end{alignat}
\end{subequations}
%
Since $V(t)$ is orthogonal for all $t\geq 0$, we see that all important qualitative properties of $x(t)$, such as boundedness, exponential stability, contractivity, expansiveness, etc., can be carried out for the function $y(t)$ without any difficulty. Clearly, we see that \eqref{eq8.2} gives an \emph{algebraic constraint} that the solution to \eqref{semi-explicit system} must obey, while \eqref{eq8.1} gives the dynamic of \eqref{semi-explicit system}. 
For this reason, we call it \emph{an underlying ODE} to \eqref{semi-explicit system}.

%
\begin{definition}
	(i) Consider the DAE \eqref{linDAE}. A matrix function $X \in C([0,\infty),\r^{n,k})$, $d\leq k \leq n$, is called \emph{a fundamental solution matrix} of \eqref{linDAE} if each of its columns is a solution to \eqref{linDAE} and $\rank X(t)=d$, for all $t\geq 0$. \\
...
Let $\{\hY_1(t,s)\}_{t\geq s \geq 0}$ be the evolution family associated with \eqref{eq8.1}, then we can define the corresponding evolution families for two DAEs \eqref{semi-explicit system}, \eqref{linDAE} consecutively as follows.
%
...
\ee
% 
Then, we can directly verify the semigroup properties, i.e.  
%
\begin{align*}
...
Furthermore, Lemmas \ref{lem4} and \ref{lem5} below show that the family $\{\hX(t,s)\}_{t\geq s\geq 0}$ does not depend on the choice of orthogonal transformations,  
and it plays the same role as the evolution family $\{X(t,s)\}_{t\geq s\geq 0}$, in comparison to \eqref{variational form}.

\begin{lemma}\label{lem4}
The families $\{X(t,s)\}_{t\geq s\geq 0}$, $\{X^-(t,s)\}_{t\geq s\geq 0}$ defined by \eqref{eq10}, \eqref{eq11} do not depend on the choice of orthogonal transformations.
\end{lemma}
...
Without loss of generality, we assume that $(E,A)$ is already in the form of the right hand side of \eqref{eq13.1}, so $U$ and $V$ in Lemma \ref{lem1} are identity matrices and $\hX(t,s)=\hY(t,s)$ for all $t\geq s \geq 0$. 
The corresponding system to the right hand side of \eqref{eq13.2} reads
%
\begin{subequations}\label{eq14}
	\begin{alignat}{2}
		\dot{\ty}_1(t) &= \hat{\cA}_1(t) \ty_1(t), \label{eq14.1} \\ 
		\ty_2(t) &= \hat{\cA}_3(t) \ty_1(t). \label{eq14.2}
	\end{alignat}
\end{subequations}
%  
...
\begin{subequations}\label{eq15}
	\begin{alignat}{2}
		& \m{\tSi & 0 \\ 0 & 0}  = \m{S_1 & S_2 \\ S_3 & S_4} \m{\Si & 0 \\ 0 & 0} \m{T_1 & T_2 \\ T_3 & T_4},  \label{15.1}\\
		& \m{\tA_1 & \tA_2 \\ \tA_3 & \tA_4} =  \m{S_1 & S_2 \\ S_3 & S_4} \left( \m{A_1 & A_2 \\ A_3 & A_4} \m{T_1 & T_2 \\ T_3 & T_4} - 
		\m{\Si & 0 \\ 0 & 0} \m{\dot{T}_1 & \dot{T}_2 \\ \dot{T}_3 & \dot{T}_4} \right) \ . \label{15.2}
	\end{alignat}
\end{subequations}
%
Let $\{\hcY_1(t,s)\}_{t\geq s \geq 0}$ be the evolution family associated with \eqref{eq14.1}, then the evolution family associated with system \eqref{eq14} is
%
...
%
From \eqref{15.1}, it implies that $S_3 \Si \m{T_1 & T_2} = 0$. Thus, we have
%
\[
\m{S_3 & 0} \m{\Si & 0 \\ 0 & I} \m{T_1 & T_2 \\ T_3 & T_4} = \m{0 & 0},
\]
%
and hence, this follows that $S_3 = 0$. Thus, $S=\sm{S_1 & S_2 \\ 0 & S_4}$, and then, due to the orthogonality of $S$, $S_1$ is nonsingular and $S_4$ is orthogonal. Also from \eqref{15.1}, we see that $S_1\Si T_2 = 0$, which yields that $T_2 = 0$. Moreover, due to the orthogonality of $S$ and $T$, from \eqref{eq14.1} we have
%
\[
  \m{\Si & 0 \\ 0 & 0}  = \m{S^T_1 & S^T_3 \\ S^T_2 & S^T_4} \m{\tSi & 0 \\ 0 & 0} \m{T^T_1 & T^T_3 \\ T^T_2 & T^T_4}.
\]
%
Therefore, using similar arguments as above, we can prove that $S_2=0$ and $T_3=0$. \\
%
Consequently, by inserting $S_3=T_3=0$ and $S_2=T_2=0$ into \eqref{15.1} and \eqref{15.2} we obtain
%
\begin{subequations}\label{eq17}
\begin{alignat}{2}
 \tSi &= S_1 \Si T_1, \\
 \m{\tA_1 & \tA_2 \\ \tA_3 & \tA_4} &= \m{S_1  \left( A_1 T_1 - \Si \dot{T}_1 \right) & S_1 A_2 T_4 \\ S_4 A_3 T_1 & S_4 A_4 T_4}  \ ,
%& \tA_1 = S_1  \left( A_1 T_1 - \Si \dot{T}_1 \right) , \\
%& \tA_2 = S_1 A_2 T_4, \\
%& \tA_3 = S_4 A_3 T_1 , \\
%& \tA_4 = S_4 A_4 T_4 \ ,
\end{alignat}
\end{subequations}
%
where the matrix-valued function $S_i$, $T_i$ ($i=1$, $4$) are pointwise orthogonal. Thus, we have
%
\begin{subequations}\label{eq18}
	\begin{alignat}{2}
		\hat{\cA}_3 &= - \tA^{-1}_4 \tA_3 = - \left( S_4 A_4 T_4 \right)^{-1} S_4 A_3 T_1 =  T_4^{-1} \hA_3 T_1 \ , \\
		\hat{\cA}_1 &= \tSi^{-1} \left( \tA_1 - \tA_2 \tA^{-1}_4 \tA_3  \right) =  T_1^{-1} \left( \hA_1 T_1 - \dot{T}_1 \right) \ .  		
	\end{alignat}
\end{subequations}
%
Furthermore, since $y = T \ty $, the structure of $T$ implies that $y_1 = T_1 \ty_1$ and $y_4 = T_4 \ty_4$. 
Therefore, the underlying ODE \eqref{eq14.1} is directly obtained from \eqref{eq8.1} by applying the variable transformation $\ty_1(t)=T_1(t) y_1(t)$ and scaling the system with $T^{-1}_1$. So, we have that $\hcY_1(t,s) = T_1^{-1} \hY_1(t,s) T_1(s)$. 
Making use of \eqref{eq18}, we can deduce the evolution family $\{\hcX(t,s)\}_{t\geq s \geq 0}$ as follows 
%
\[
 \hcX(t,s)  = \m{T_1(t) & 0 \\ 0 & T_4(t)} \m{\hcY_1(t,s) & 0 \\ \hcA_3 \hcY_1(t,s) & 0} \m{T^T_1(s) & 0 \\ 0 & T^T_4(s)} = \m{\hY_1(t,s) & 0 \\ \hA_3 \hY_1(t,s) & 0},
\]
%
...
Consider the DAE \eqref{semi linDAE} and the evolution family $(X(t,s))_{t\geq s\geq 0}$ defined by \eqref{eq10}. Furthermore, we also consider the pointwise-orthogonal matrix-valued functions $U$, $V$ defined in Lemma \ref{lem5}. Then, the solution to \eqref{semi linDAE}, if exists, also satisfies the so-called \emph{mild equation}
	%
	\begin{align*}
		\m{x_1(t) \\ x_2(t)} =& \hX(t,s) \m{x_1(s) \\ x_2(s)} + \int_{s}^{t} \hX(t,\tau) \m{\Si^{-1}(\tau) & -\Si^{-1}(t) A_2(t) A^{-1}_4(t) \\ 0 & 0} U(\tau) f(\tau,x(\tau)) d\tau  \\
		&\ + \m{0 & 0 \\ 0 & -A^{-1}_4(t) } U(t) f(t,x(t)),
	\end{align*}
...
In the following, for ease of notation, we will use the abbreviation $\hX(t):=\hX(t,0)$, $\hX^-(t):=\hX^-(t,0)$, $\hY(t):=\hY(t,0)$ and $\hY^-(t):=\hY^-(t,0)$. The concept of exponential dichotomy for the DAE \eqref{semi-explicit system} is given as below.

\begin{definition}\label{def dichotomy}(\cite{LinM09}) The DAE \eqref{semi-explicit system} is said to have an \emph{exponential dichotomy} if there exist a family of projection matrices $\{\Py(t)\}_{t\geq 0}$ in $\r^{d,d}$ and positive constants $N$, $\nu$ such that
%
...
Since the Euclidean norm is preserved under orthogonal transformations, due to \eqref{eq10} and \eqref{eq19} we have
%
\begin{equation*}
...
\eqref{linDAE} as the one of \eqref{semi-explicit system}. 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%  Section 2%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{definition}[\cite{Huy06}]\label{defE1}
	...
	\begin{enumerate}[(i)]
		...
		\frac{M(b-a)}{\|\chi_{[a,b]}\|_E}\|\varphi\|_E\hbox{ for all } \varphi\in E,
		\end{equation}
		where $\chi_{[a,b]}$ is the indicator function of $[a,b]$. 
...
\]
%
and hence, $E\hookrightarrow {\mathbf M}_1(\r_+)$. Furthermore, $C_{b}(\r^+)$, the Banach space of bounded, continuous function from $\r_+$ to $\r^n$, is dense in ${\mathbf M}_1$.
\end{remark}
...
In the qualitative analysis of ODEs, one of the central topics is to find necessary and sufficient conditions such that the considered system admits an exponential dichotomy. Many researches have been devoted to this topic, and critical results have been achieved for ODEs  in finite and infinite dimensional phase spaces (e.g. \cite[Chap. 4]{DalK02}, \cite{Zhi72}). For DAEs, the only result that we are aware of is recalled below.

\begin{proposition}\label{prop1} (\cite{LinM09})
The DAE \eqref{semi-explicit system} has an exponential dichotomy if and only if the corresponding underlying ODE \eqref{eq8.1} also has exponential dichotomy and the matrix function $\hA_3(t)$ is bounded. Moreover, the existence of an exponential dichotomy implies that $\underset{t\geq 0}{\sup \ } \|\Py(t)\| < \infty$. 
\end{proposition}

Notice that, Proposition \ref{prop1} is only valid for finite-dimensional but it is very hard to generalize for infinite dimensional DAE systems.  
For this reason, we aim at another approach, motivated from one classical result stated below.
%
\begin{proposition}\label{prop2} (\cite[Chap.3]{Cop78}) The ODE \eqref{eq3} has an exponential dichotomy if and only if one of the following conditions
is satisfied. \\
i) For any function $g\in  \mathbf{M}_{1}(\r_+)$ there exists a continuous, bounded solution $x(t)$ to the inhomogeneous system
%
\begin{equation}\label{inho ODE}
	\dot{x}(t) = A(t) x(t) + g(t) \ .  
\end{equation}
%
ii) For any function $g \in C_{\infty}(\r_+)$, there exists a continuous, bounded solution $x(t)$ to the inhomogeneous system \eqref{inho ODE}, provided that 
the ODE \eqref{eq3} has bounded growth.	
\end{proposition}
%
Comparable results to Proposition \ref{prop2} have not been achieved for DAEs, and hence, this will be our main aim in this section.
Together with \eqref{linDAE}, let us consider the following system
%
\begin{equation}\label{eq2.1}
	E(t)\dot{x}(t) = A(t) x(t) + g(t) \ .  
\end{equation}
%
Since the existence of an exponential dichotomy is a characteristic property of the evolution family $\{\hX(t,s)\}_{t\geq s\geq 0}$, which is invariant with respect to orthogonal transformations, we may assume that system \eqref{eq2.1} is already in the semi-explicit form \eqref{semi-explicit system}. 
The following example shows that Proposition \ref{prop2} could not be directly applied to the DAE \eqref{eq2.1}.
\begin{example}\label{Exam2.1}
Consider the system \eqref{eq2.1} with $E=\sm{1 & 0 \\ 0 & 0}$, $A=\sm{-1 & 0 \\ 0 & e^{-t}}$, $f=\sm{0 \\ 1}$. Clearly, $f$ is bounded. 
On the other hand, the homogeneous system clearly has an exponential dichotomy.
Nevertheless, the explicit solution $x(t) = \sm{e^{-t}x_1(0) \\ e^{t}}$ is unbounded no matter how an initial condition $x(0)$ is chosen.  
\end{example}

%\begin{definition}\label{EA-bounded}
%	Consider the matrix functions $E$, $A$ in system \eqref{linDAE}. Then, a function $g:\r_+ \rar \r^n$ satisfying the condition 
%	%
%	\begin{equation}\label{eq2.2}
%	\underset{t\geq 0}{\sup} \bigg\{ \bigg\| \m{\Si^{-1}(t) & -\Si^{-1}(t) A_2(t) A^{-1}_4(t) \\ 0 & A^{-1}_4(t)} g(t) \bigg\| \bigg\} < +\infty,
%	\end{equation}
%	%
...
We define the linear space $\Csys$ associated with the system \eqref{linDAE} as follows.
%
\begin{equation}\label{eq2.2}
C_b^{sys}(\r_+):=\left\{g \in C(\r_+) \ \Bigg| \ \underset{t\geq 0}{\sup} \bigg\{ \bigg\| \m{\Si^{-1}(t) & -\Si^{-1}(t) A_2(t) A^{-1}_4(t) \\ 0 & A^{-1}_4(t)} g(t) \bigg\| \bigg\} < +\infty \right\} \ ,
\end{equation}
%

\begin{remark}
In Theorem \ref{thm3.1} below we will see that $\Csys$ plays the role of 
$C_b(\r_+)$  in Proposition \ref{prop1}. In fact, for the ODE case, $A_4$ is an empty matrix and $\Si=I_n$, so $\Csys$ coincides with $C_b(\r_+)$. 
\end{remark}

\begin{lemma}\label{lem2.4}
The space $\Csys$ is invariant with respect to system orthogonal transformations.  
\end{lemma}
\begin{proof} Let us consider two orthogonally equivalent systems
\begin{align*}
	& E(t) \dot{x}(t) = A(t) x(t) + g(t), \\
	& \tE(t) \dot{\tx}(t) = \tA(t) \tx(t) + \tg(t),
\end{align*}
%
where $(E,A,g) \osim (\tE,\tA,\tg)$, and 
%
\[
(E,A) = \left( \m{\Si & 0 \\ 0 & 0},\m{A_1 & A_2 \\ A_3 & A_4} \right), \mbox{ and } (\tE,\tA) = \left( \m{\tSi & 0 \\ 0 & 0},\m{\tA_1 & \tA_2 \\ \tA_3 & \tA_4} \right) \ .
\]
%
As shown in the proof of Lemma \ref{lem4}, the identities \eqref{eq17} hold true for some pointwise orthogonal matrix-valued functions $S_i$, $T_i$ ($i=1$, $4$). 
Thus, we see that
%
\begin{align*}
\m{\tSi^{-1} & -\tSi^{-1} \tA_2 \tA^{-1}_4 \\ 0 & \tA^{-1}_4} \tg &= 
\m{T^T_1 & 0 \\ 0 & T^T_4} \m{\Si^{-1} & -\Si^{-1} A_2 A^{-1}_4 \\ 0 & A^{-1}_4} \m{S^T_1 & 0 \\ 0 & S^T_4} S g \\
&= \m{T^T_1 & 0 \\ 0 & T^T_4} \m{\Si^{-1} & -\Si^{-1} A_2 A^{-1}_4 \\ 0 & A^{-1}_4} g \ .
\end{align*}
%
Since Euclidean norm is preserved under orthogonal transformation, this identity completes the proof.
\end{proof}

The main result of this section is to prove a characterization of the exponential dichotomy for DAEs. Roughly speaking, the DAE \eqref{linDAE} admits exponential dichotomy if and only if the mapping $\cL := E\dfrac{d}{dt} - A$ is surjective on the space $\Csys$. 
We formulate our main result in this section as follows.
\begin{theorem}\label{thm3.1}
	Consider the linear, strangeness-free DAE \eqref{linDAE} and the associated inhomogeneous DAE \eqref{eq2.1}. 
	%Then, the DAE \eqref{linDAE} admits exponential dichotomy if and only if for any continuous, $(E,A)$-bounded function $g(t)$ on $[0,\infty)$, there exists a continuous, bounded solution $x(t)$ to the DAE \eqref{eq2.1}.
	Then the following assertions hold. 
	\begin{enumerate}[(i)]
	\item If the DAE \eqref{linDAE} admits an exponential dichotomy then for any function $g \in \Csys$, there exists a continuous, bounded solution $x(t)$ to the DAE \eqref{eq2.1}. 
	\item If the matrix function $\hA_3(t)$ is bounded, then the converse of assertion (i) holds true.
	\end{enumerate}
\end{theorem}

\begin{proof}
	Firstly, we notice that, since $\hg(t) = \sm{\Sigma^{-1}(t) & -\Si^{-1}(t) A_2(t) A^{-1}_4(t) \\ 0 & -\hA_4^{-1}} g(t)$, so the fact $g \in \Csys$ implies the boundedness of $\hg$. Recall that the semi-explicit system \eqref{semi-explicit system} reads
	%
	\begin{subequations}\label{eq3.10}	
	\begin{alignat}{2}
		\label{eq3.10a} \dot{y}_1(t) &= \hA_1(t) y_1(t) + \hg_1(t), \\
		\label{eq3.10b} y_2(t) &= \hA_3(t) y_1(t) + \hg_2(t).
	\end{alignat}
	\end{subequations}
	%
	\noindent (i) Assuming that the DAE \eqref{linDAE} admits an exponential dichotomy, then \eqref{semi-explicit system} also has an exponential dichotomy. Proposition \ref{prop1} implies that equation \eqref{eq3.10a} has an exponential dichotomy, and the function $\hA_3$ is bounded. Therefore, Proposition \ref{prop2} implies that $y_1$ is bounded, and consequently, $y_2$ is also bounded.   \\
	%
	(ii) Notice that the mapping 
	%
	\[
		g \mapsto \m{\Sigma^{-1}(t) & -\Si^{-1}(t) A_2(t) A^{-1}_4(t) \\ 0 & -A_4^{-1}}g 
	\]
	%
	is a surjection from $\Csys$ to $C_{b}(\r_+,\r^n)$, so $g(t)$ can be freely chosen in the space $C_{b}(\r_+,\r^n)$. Proposition \ref{prop2} applied to system \eqref{eq3.10a} follows that \eqref{eq3.10a} has exponential dichotomy. 
	If, in addition, the boundedness of $\hA_3$ is presumed then Proposition \ref{prop1} implies that system \eqref{linDAE} admits exponential dichotomy.
\end{proof}

% We further notice, that the boundedness of $\hA_3$ is only the sufficient but not necessary condition so that the converse of Theorem \ref{thm3.1}i) holds true. 
% The following example demonstrates this fact. 

%\begin{remark}
%Making use of admissible function spaces, stronger conditions for characterizing the exponential dichotomy of the DAE \eqref{linDAE} have been obtained in \cite{HuyP19b}, where an inhomogeneity function $g(t)$ belongs to less restricted spaces than $\Csys$. There, we also study the robustness of exponential dichotomy under structured-perturbations.%
%\end{remark}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%  Section 4%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Local Stable manifolds for semi-linear DAEs}\label{Sec 4}

In this section we study the existence of a local stable manifold for the semi-linear DAE  \eqref{semi linDAE}. 
Throughout this section we assume the following.

\begin{assumption}\label{Ass3.0}
Assume that the evolution family  $(X(t,s))_{t\geq s\geq 0}$ associated with the linear, homogeneous DAE \eqref{linDAE} admits an exponential dichotomy on $\hro{R_+}$
with the dichotomy constants $N,\nu>0$ as in Definition \ref{def dichotomy}. 
\end{assumption}

Due to Proposition \ref{prop1}, let us denote by $H_1 := \underset{t\geq 0}{\sup \ } \|\hA_3(t)\|$ and $H_2:= \underset{t\geq 0}{\sup \ } \|\Py(t)\|$.
%
In the following example, we illustrate the sensitivity of an exponential dichotomy and a \emph{constraint manifold}
%
\be\label{constraint manifold}
\bbL(t,x):= \{(t,x) \in \r_+ \times \r^n \ | \  \mathbf{A}_2(t) x(t) = 0 \}   \ . 
\ee
%
with respect to small disturbance $f(t,x)$.

\begin{example}\label{Example 3.1}
Consider the disturbed DAE with the parameter function $\a(t)$
%
\begin{equation}\label{eq3.2}
\m{1 & 0 & 0 \\ 0 & 1 & t \\ 0 & 0 & 0} \dot{x}(t) = \m{-2 & 0 & 0 \\ 0 & 2 & \a(t) \\ 0 & 0 & 1} x(t) + f(t,x) \ \mbox{ for all } t \geq 0 \ . 
\end{equation}
%
Clearly, we see that the homogeneous system has an exponential dichotomy with the projection $P_x(t) \!=\! \sm{1 & 0 & 0 \\ 0 & 0 & 0 \\ 0 & 0 & 0}$. 
Moreover, every solution to the homogeneous system must remain on the constraint manifold
%
\[ \bbL(t,x) = \{x = \sm{x_1 & x_2 & x_3}^T \in \r^3 \ | \ x_3 = 0\} \ . 
\]
%
For an arbitrarily large $t_1 > 0$, we consider the disturbance $f(t,x) = \m{0 & 0 & 0}^T$ for $t<t_1$ and $f(t,x) = \m{0 & 0 & \frac{x_2}{t}}^T$ for $t\geq t_1$. Thus, for $t\geq t_1$ system \eqref{eq3.2} becomes
%
\begin{subequations}\label{eq3.3}
	\begin{alignat}{4}
		\label{eq3.3a} \dot{x}_1 &= -2 x_1 ,  \\
		\label{eq3.3b} \dot{x}_2 + t \dot{x}_3 &= 2 x_2 + \a(t) x_3 , \\
		\label{eq3.3c} 0 &= \frac{1}{t} x_2 + x_3 \ . 
	\end{alignat}
\end{subequations}
%
Scaling equation \eqref{eq3.3c} with $t$, and differentiating it, we obtain $\dot{x}_2 + t \dot{x}_3 + x_3 = 0$. Then, substituting $\dot{x}_2 + t \dot{x}_3 = -x_3$ into \eqref{eq3.3b} we achieve the \emph{hidden constraint} $0 = 2 x_2 + (1+\a(t))x_3$.
Consequently, the disturbance $f(t,x)$ above has drastically changed both the dynamic and the constraint of the homogeneous system. 
Now the constraint manifold is
%
\[
\bbL(t,x) = \{ x = \sm{x_1 & x_2 & x_3}^T \in \r^3 \ | \ \sm{2 & 1+\a(t) \\ 1 & t} \sm{x_2 \\ x_3} = 0 \}  \ \mbox{ for all } t \geq t_1 \ ,
\]
%
and the dynamic part involves only $x_1$ which is exponential stable. In the worst case, if $\a(t)=2t-1$ then system \eqref{eq3.2} is not even uniquely solvable.
\end{example}

\begin{remark}
Example \ref{Example 3.1} demonstrates one fact different to the ODE theory, that even though the disturbance $f(t,x)$ satisfies the Lipschitz property with an arbitrarily small Lipschitz constant (since $t_1$ can be chosen to be arbitrarily large), the dynamic and constraints of the system may be drastically altered. Therefore, to study the existence of invariant manifolds for \eqref{semi linDAE}, it is necessary to presume some additional assumptions, in order to guarantee, that both the dynamic and the constraint manifold do not change too much with respect to disturbance.
\end{remark}

As in Lemma \ref{lem1}, by using orthogonal transformation $x(t)=V(t)y(t)$ and scaling system \eqref{semi linDAE} with $U(t)=\sm{U_1(t) & 0 \\ 0 & I_a}$, where $y(t)= \sm{y_1(t) \\ y_2(t)}  \in \r^{d+a}$, we derive the coupled system
%
\begin{subequations}\label{eq3.4}
	\begin{alignat}{2}
		\label{eq3.4a} \dot{y}_1(t) &= \hA_1(t) y_1(t) + \hf_1(t,y(t)), \\
		\label{eq3.4b} y_2(t) &= \hA_3(t) y_1(t) + \hf_2(t,y(t)),
	\end{alignat}
\end{subequations}
%
where
...
\hf(t,y(t)) = \m{\hf_1(t,y(t)) \\ \hf_2(t,y(t)) } := 
\m{\Sigma^{-1}(t) & -\Si^{-1}(t) A_2(t) A^{-1}_4(t) \\ 0 & -A_4^{-1}} \m{U_1(t) f_1(t,x(t)) \\ f_2(t,x(t))} \ .
\end{equation*}
%
Notice that equation \eqref{eq3.4b} only gives an implicit algebraic constraint in terms of $y_1$ and $y_2$. 
...
%
\[
\n{ A^{-1}_4(t) \left( f_2(t,x) - f_2(t,\tx) \right) } \leq L \|x-\tx \|, 
\]
%
...
\begin{lemma}\label{lem3.0} Let $\rho_0 := \sqrt[2]{1+ \left( \frac{H_1+L}{1-L}\right)^2} \ \rho$ \ and \ $\brho_0 := \frac{H_1+L}{1-L}\rho_0$. \ 
Under Assumption \ref{Ass3.1} and any given $y_1\in B_{\rho_0}$, there exists a unique function $y_2\in B_{\brho_0}$ satisfying  \eqref{eq3.4b}. Consequently, $y \in B_{\rho}$.
%restricted to the ball $B_{\rho} \subset \r{^n}$, $y_2$ can be uniquely solvable from \eqref{eq3.4b} in terms of $t$ and $y_1$.
\end{lemma}
%
\begin{proof}
Firstly, notice that Assumption \ref{Ass3.1} implies that
%
\[
\n{\hf_2(t,y) - \hf_2(t,\ty)} = \n{-A_4^{-1} \left( f_2(t,Vy) - f_2(t,V\ty) \right)}
\leq L \n{Vy - V\ty} = L \n{y-\ty} \ .
\]
% 
Thus, $\hf_2(t,y)$ is also Lipschitz in $y$ with the same constant $L$. 
Then, the desired claim is obtained directly by making use of \cite[Lem. 2.7]{MinW04}.
\end{proof}

%\begin{remark}\label{rem3.1}
%Lemma \ref{lem3.0} leads to one important fact, that under Assumption \ref{Ass3.1}, the coupled system \eqref{eq3.4a}-\eqref{eq3.4b} is still strangeness-free, as defined in \cite[Chap. 4]{KunM06}. Therefore, in analogue to the linear case, \eqref{eq3.4b} is called \emph{an algebraic constraint}, whereas \eqref{eq3.4a} is called \emph{an underlying ODE}.
%\end{remark}

%To obtain the existence of a stable manifold 
We also need the following property of the nonlinear part $f_1$ defined as follows.
%
...
\end{definition}
%
\begin{assumption}\label{Ass3.2}
Assume that the function \ $t\mapsto \Sigma^{-1}(t) \m{I_d & -A_2(t) A^{-1}_4(t)} f(t,x(t))$  belongs to class $(M,\vphi,\rho)$ for some positive constants $M$, $\rho$ and a positive function $\vphi \in E$.
\end{assumption}

The following lemma shows that Assumptions \ref{Ass3.1}, \ref{Ass3.2} are invariant with respect to system orthogonal transformations. 
%
\begin{lemma}\label{lem3.2}
Assumptions \ref{Ass3.1}, \ref{Ass3.2} are also invariant with respect to system orthogonal transformations. 
\end{lemma}
%
\begin{proof}
Let us consider two orthogonally equivalent systems
%
\[
\left( \m{\Si & 0 \\ 0 & 0},\m{A_1 & A_2 \\ A_3 & A_4} \right) \osim \left( \m{\tSi & 0 \\ 0 & 0},\m{\tA_1 & \tA_2 \\ \tA_3 & \tA_4} \right) \ .
\]
%
As shown in the proof of Lemma \ref{lem4}, the identities \eqref{eq17} hold true for some pointwise orthogonal matrix-valued function $S_i$, $T_i$ ($i=1$, $4$). 
Therefore, we have that
%
\begin{align*}
\| \tA^{-1}_4(t) \left( \tf_2(t,x) - \tf_2(t,\tx) \right) \| 
= \n{T_4^{-1}(t)  A^{-1}_4(t)  S_4^{-1}(t) S_4(t) \left( f_2(t,x) - f_2(t,\tx) \right) } 
\leq L \|x-\tx \|, 
\end{align*}
%
...
%
\[
\tSi^{-1} \m{I_d & -\tA_2 \tA_4^{-1}} = T_1 \Si^{-1} \m{I_d & -A_2 A_4^{-1}} \m{S_1 & 0 \\ 0 & S_4}^T \ ,
\]
%
and hence 
%
\[
\tSi^{-1} \m{I_d & -\tA_2 \tA_4^{-1}} \left( \tf_2(t,x) - \tf_2(t,\tx) \right) = T_1 \Si^{-1} \m{I_d & -A_2 A_4^{-1}} \left( f(t,x) - f(t,\tx) \right) \ . 
\]
%
Then, due to the orthogonality of $T_1$, $T_4$, $S_1$, $S_4$, the desired claims are directly followed.
\end{proof}

The following lemma gives one sufficient condition for Assumptions \ref{Ass3.1}, \ref{Ass3.2}.

\begin{proposition}\label{pro3.9}
Consider the semi-linear DAE \eqref{semi linDAE}. Then, the following claims hold true. \\
i) Assume that the function $\n{\Si^{-1}\m{I_d & -A_2 A^{-1}_4}}$ is bounded by $\mathbf{M}$. If $f$ belongs to the class $(M,\dfrac{\vphi}{\mathbf{M}},\rho)$ then $\hf_1$ belongs to the class $(M,\vphi,\rho)$. \\
ii) Assumption \eqref{Ass3.1} holds true of $f(t,x)$ satisfies that inequality 
%
\begin{equation}\label{Lipschitz}
	\| f_2(t,x) - f_2(t,\tx) \| \leq \dfrac{L}{\n{A^{-1}_4(t)}} \|x-\tx \|  \ .
\end{equation}
%
\end{proposition}

For the simplicity of presentation, we will study the existence of a local stable manifold for system \eqref{eq3.4a}-\eqref{eq3.4b}. 
...
Let Assumptions \ref{Ass3.1} and \ref{Ass3.2} hold true. Then, for all $y, \ty \in B_{\rho}$ the following assertions hold. \\ 
...
\end{lemma}
\begin{proof}
The proof is trivially followed from Assumptions \ref{Ass3.1} and \ref{Ass3.2} due to the fact that $\n{y}=\n{Qy}$ for any orthogonal matrix $V$.
\end{proof}

Under Assumption \ref{Ass3.0}, the family $(\hY(t,s))_{t\geq s\geq 0}$ also has an exponential dichotomy with the corresponding projection matrices $\{\Py(t)\}_{t\geq 0}$ and the same dichotomy constants $N,\nu>0$. Thus, we can define the Green function on the half-line as follows
...
%
\begin{lemma}\label{lem3.3} Assume that Assumptions \ref{Ass3.0}, \ref{Ass3.1}, \ref{Ass3.2} hold true. 
...
for all $t\geq t_0$. \ 
%
Now let us estimate $\n{z(t)}$. Making use of Lemma \ref{lem3.1} and \eqref{4.3}, we see that 
%
\[
\n{z(t)} \leq \int_{t_0}^{\infty} (1+H_1)(1+H_2) \ Ne^{-\nu |t-\tau|} \ M \vphi(\tau) \ d\tau \ + L\rho, 
\]
and then, from \eqref{eq22} it follows that
%
	\[
...
\]
%
we see that $v_0:= z_1(t_0) - y_1(t_0) \in \im \Py(t_0)$. Finally, since $z(t)=y(t) + \hY(t,t_0) \sm{v_0 \\ 0}$ for all $t\geq t_0$, equality \eqref{4.4} follows.
\end{proof}

\begin{remark}\label{rm3.4}
By computing directly, we can see that the converse of Lemma  $\ref{lem3.3}$ is also true. It means, that all solutions to $\eqref{4.4}$ also satisfy equation  $\eqref{mild equation}$ for all $t\geq t_0.$  
\end{remark}
...
\begin{lemma}\label{lem3.4}
	Under the assumptions of Lemma \ref{lem3.3}, let $y(t)$, $\ty(t)$ be any two functions lying in the ball $B_{\rho}$ and satisfy \eqref{4.4} for $v_0$, $\tv_0 \in \im \Py(t_0)$. If $H_3$ defined as in \eqref{4.8} satisfies $H_3+L<1$ then the following estimate holds true:
	%
...
\end{lemma}
\begin{proof}
Using the same arguments as in the proof of Lemma \ref{lem3.1}, we see that 
%
\begin{align*}
...
\begin{theorem}\label{thm4.1} Consider the semi-explicit \eqref{semi linDAE} and the transformed system \eqref{eq3.4}. Furthermore, assume that Assumptions \ref{Ass3.0}, \ref{Ass3.1}, \ref{Ass3.2} hold true, and constant $H_3$ defined as in \eqref{4.8}. Then, the following assertions hold true.\\
(i) If  
%
...
%
Using the same argument as in the proof of Lemma \ref{lem3.1}, we see that
%
\bens
...
%First observe that due to Assumption \ref{Ass3.1} and the definition of $\hf_2$, we have
%%
%\[
...
%We define the so-called \emph{constrained manifold}, which all the solution to \eqref{semi linDAE} must lie on
%%
%\[
%\bbL(t,x):= \{(t,x) \in \r_+ \times \r^n\ | \  0 = A_2(t)x(t) + f_2(t,x(t)) \}  \ . 
%\]
%%
%We further notice that this manifold is of dimension $d$, which is the degree of freedom to the DAE \eqref{semi linDAE}. 
...
Let 
%
\be\label{constraint manifold of y}
\bbL(t,y):= \{(t,y_1,y_2) \in \r_+ \times \r^d \times \r^a\ | \  y_2 = \hA_3(t)y_1 + \hf_2(t,y_1,y_2) \}  \ ,
\ee
%
be the constrained manifold, which all solutions to \eqref{eq3.4a}-\eqref{eq3.4b} must belong to. \ 
%
...
Consider the semi-linear DAE \eqref{semi linDAE} and the semi-explicit system \eqref{eq3.4}. Furthermore, assume that Assumptions \ref{Ass3.0}, \ref{Ass3.1}, \ref{Ass3.2} hold true. If 
%
...
%
For any $\rho>0$ defined as in  Assumptions \ref{Ass3.1}, \ref{Ass3.2}, let $\rho_1 := \dfrac{1-L}{2N(1+H_1)} \rho$ \ and \ $\rho_2 := \dfrac{(1-L)\rho}{2}$. For each $t \geq 0$ we define the mapping $g_{t}$ acting on $B_{\rho_1} \cap W_1(t)$ as 
%
\[
...
Now, we prove that $\n{g_{t}(w_1)} \leq \rho_2$. Due to Theorem \ref{thm4.1} (i) and Lemma \ref{lem3.1} (i), we have that $\n{y(t)} \leq \rho$ and $\n{f_1(\tau,y(\tau))} \leq M \vphi(\tau)$ for a.e. $t\geq 0$. Therefore,
%
\bens
...
In this section we study the existence of global stable manifolds for semi-linear DAEs of the form \eqref{semi linDAE}. We begin with the concept of $\vphi$-Lipschitz functions.
...
In comparability to Assumptions \ref{Ass3.1}, \ref{Ass3.2}, we also need some global properties of the nonlinear term $f$.

\begin{assumption}\label{Ass5.1}
Assume that the following hypotheses hold true.
\begin{enumerate}[(i)]
\item  The function \ $t\mapsto \Sigma^{-1}(t) \m{I_d & -A_2(t) A^{-1}_4(t)} f(t,x(t))$ \ is $\vphi$-Lipschitz. 
\item The function $A^{-1}_4(t) \ f_2(t,x(t))$ is a contraction mapping with the Lipschitz constant $L<1$ for all $(t,x(t))$ lying on the constraint-manifold associated with \eqref{semi linDAE} defined by
\begin{equation*}
\bbL(t,x):= \{(t,x) \in \r_+ \times \r^n \ | \   \mathbf{A}_2(t)x + f_2(t,x)=0 \}  \ . 
\end{equation*}
\end{enumerate}
\end{assumption}

In analogous to Lemma \ref{lem3.2}, we can directly verify that Assumption \ref{Ass5.1} is invariant with respect to system orthogonal transformations. 
Hence, function $\hf_1$ in \eqref{eq3.4a} is $\vphi$-Lipschitz, and function $\hf_2$ in \eqref{eq3.4b} is a contraction mapping with the Lipschitz constant $L<1$. \ 
For notational simplicity, now we will study the semi-explicit system \eqref{semi-explicit system} and the integral-algebraic system \eqref{mild equation}.
...
Analogously to Lemma \ref{lem3.3}, we give the explicit form of bounded solutions to system \eqref{mild equation} as below.
%
\begin{lemma}\label{lem5.1}
	Let the evolution family  $(\hY(t,s))_{t\geq s\geq 0}$ of system \eqref{semi-explicit system} have an exponential dichotomy with the corresponding projection matrices $\{\Py(t)\}_{t\geq 0}$  and the dichotomy constants $N,\nu>0$. Furthermore, assume that Assumption \ref{Ass5.1} holds true. 
...
\end{lemma}
\begin{proof}
The proof can be done by using similar arguments as in the proof of Lemma \ref{lem3.0}. 	
\end{proof}
...
\begin{theorem}\label{thm5.1}
	Let the evolution family  $(\hY(t,s))_{t\geq s\geq 0}$ of system \eqref{semi-explicit system} have an exponential dichotomy with the corresponding projection matrices $\{\Py(t)\}_{t\geq 0}$  and the dichotomy constants $N,\nu>0$. Furthermore, assume that Assumption \ref{Ass5.1} holds true. 
	\begin{enumerate}[(i)]
	\item For any fixed $t_0 \geq 0$, if $H_3<1-L$ 
	%
%	\begin{equation*}
%	%\dfrac{H_3}{1-e^{-\nu}} 
%	H_3 <  1-L,
...
\begin{theorem}\label{thm5.2}
	...
	%
	\begin{equation}\label{condition H3}
	%\dfrac{H_3}{1-e^{-\nu}} 
...
\begin{example}\label{exam5.1}
		...
		%
		\begin{equation*}
		\m{M & 0 \\ 0 & 0} \m{\dot{U} \\ \dot{P}} = \m{K & -C \\ C^T & 0} \m{U \\ P} + \m{F(t,U,P) \\ 0},
		\end{equation*}
		%
		where $U(t)$, $P(t)$ approximate the velocity $u(t,\xi)$ and the pressure $p(t,\xi)$, respectively. Here the leading matrix $M$ is either an identity matrix or a symmetric positive definite matrix depending on the spatial discretization scheme, $K$ and $C$ are constant matrices of appropriate dimensions. 
		Furthermore, in many applications, the matrix $C^T M^{-1} C$ is nonsingular. 
		We notice, that this system also occurs in many fluid dynamic models, see e.g. \cite{EmmM13,AltH18}. 
		It is well-known, that the differentiation index of this system is two, and hence, it is not strangeness-free, so Assumption \ref{Ass1.0} is violated. 
		Nevertheless, we can transform it first in order to obtain a strangeness-free DAE
		%
		\begin{equation}\label{eq5.3}
		\m{M & 0 \\ 0 & 0} \m{\dot{U} \\ \dot{P}} = \m{K & -C \\ C^TM^{-1}K & - C^TM^{-1}C} \m{U \\ P} + \m{F(t,U,P) \\ C^TM^{-1} F(t,U,P)},
		\end{equation}
		%
		Fortunately, we do not need any system transformation, since \eqref{eq5.3} is already in the semi-explicit form. 
		%
		Since $M$, $K$, $C$ are constant matrices, due to Proposition \ref{prop1}, the homogeneous DAE 
		%
		\[
		\m{M & 0 \\ 0 & 0} \m{\dot{U} \\ \dot{P}} = \m{K & -C \\ C^TM^{-1}K & -C^TM^{-1}C} \m{U \\ P}
		\]
		%
		admits an exponential dichotomy if and only if the matrix $\hA_1 := M^{-1} \left(I - C \left( C^T M^{-1} C \right)^{-1} C^T M^{-1} K \right) $ is hyperbolic. \
		%
		If, in addition, $F(t,U,P)$ satisfies the $\dfrac{\vphi}{\mathbf{M}}$-Lipschitz condition, where 
		%
		\[
		\mathbf{M}:=\n{M^{-1} \left( I - C \left( C^T M^{-1} C \right)^{-1} C^T M^{-1} \right)}
		\]
		%		
		and $F(t,U,P)$ is a contraction mapping with the Lipschitz constant $L < \dfrac{1}{\n{ \left( C^T M^{-1} C \right)^{-1} C^T M^{-1} }}$, 
		then Theorem \ref{thm5.2} and Proposition \ref{pro3.9} imply that there exists a stable manifold for the solution to \eqref{eq5.3}, whenever the function $\vphi$ satisfies the condition \eqref{condition H3}.
\end{example}

\begin{example}\label{exam5.2} Motivated from \cite{Ria04}, we consider the nonlinear electrical circuit with  Josephson junction in Figure \ref{josephson junction} below. The Josephson junction device on the right hand side, consisting of two super conductors separated by an oxide barrier, is characterized by the sinusoidal relation $i_2 = I_0 \sin(k\phi_2)$, where $I_0$ and $k$ are positive constants depend on the device itself. Moreover, the resistance $R$, inductance $L$ and conductance $G$ are positive. Furthermore, $i_1$ is the current going through the inductance, $v_1$ and $v_2$ are voltage drops across  the inductance and the Josephson junction, respectively.
%
\begin{figure}[h!]
	\centering
...
\end{figure}
%	
As shown in \cite{Ria04}, for the linear devices (R, L, C), we obtain the following system, which completely describes the behavior of this circuit. 
%
\begin{equation}\label{eq5.5}
\m{1 & 0 & 0 \\ 0 & 1 & 0 \\ 0 & 0 & 0} \m{\dot{\phi}_1 \\ \dot{\phi}_2 \\ \dot{v}_2}  = \m{\frac{R}{L} & 0 & 1 \\ 0 & 0 & 1 \\ \frac{1}{L} & I_0 k & G} \m{\phi_1 \\ \phi_2 \\ v_2}
\end{equation}
%
This system posses an exponential dichotomy for any $k<0$. In case of nonlinear devices, we obtain a nonlinear system
%%
%\begin{subequations}
%\begin{alignat}{4}
...
\begin{equation}\label{eq5.6}
	\m{1 & 0 & 0 \\ 0 & 1 & 0 \\ 0 & 0 & 0} \m{\dot{\phi}_1 \\ \dot{\phi}_2 \\ \dot{v}_2} = \m{\frac{R}{L} & 0 & 1 \\ 0 & 0 & 1 \\ \frac{1}{L} & I_0 k & G} \m{\phi_1 \\ \phi_2 \\ v_2}
	+ \m{f_1(t,\phi_1) \\ 0 \\ f_2(t,\phi_1,v_2)} \ .
\end{equation}
%
Fortunately, we do not need any system transformation, since \eqref{eq5.3} is already in the semi-explicit form.
In this case, we have that $A_4 = 1$, $\m{I -A_2A_4^{-1}} = \m{1 & 0 & -1/G \\ 0 & 1 & -1/G}$, and hence for any $\vphi$-Lipschitz function $f_1$ and any contraction mapping $f_2$, we obtain a stable manifold for \eqref{eq5.6} whenever the condition \eqref{condition H3} is satisfied.
\end{example}
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
