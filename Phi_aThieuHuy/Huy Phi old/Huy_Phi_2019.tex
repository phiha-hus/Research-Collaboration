...
\title[EXPONENTIAL DICHOTOMY AND STABLE MANIFOLD FOR DAEs]{ON EXPONENTIAL DICHOTOMY AND STABLE MANIFOLDS FOR DIFFERENTIAL-ALGEBRAIC EQUATIONS ON THE HALF-LINE}
...
\begin{abstract}
In this work we study linear/semi-linear differential-algebraic equations (DAEs) on the half-line $\r_+$. First we characterize the existence of exponential dichotomy for linear DAEs by invoking the Lyapunov-Perron method. Then we prove the existence of local and global, invariant, stable manifolds for semi-linear DAEs in the case that the corresponding evolution family to an associated linear DAE admits exponential dichotomy and an inhomogeneity function fulfills the non-uniform $\vphi$-Lipschitz condition, where the Lipschitz function $\vphi$ belongs to wide classes of admissible function spaces such as $L_p$, $1\leq p \leq \infty$, $L_{p,q}$, etc.  
\end{abstract}
...
Our focus in the present paper is on the existence of (local and global) stable manifolds for semi-linear time varying differential-algebraic equations (DAEs) of the form
%
...
\end{equation}
%
Beside that, we also study the exponential dichotomy of the associated linear system
%
\begin{equation}\label{eq2}
E(t)\dot{x}(t) = A(t) x(t), \quad t\in [0,+\infty).
\end{equation}
%
Here $E=\m{E_1(t) \\ 0}$, $A = \m{A_1(t) \\ A_2(t)}$ are assumed to be matrix-valued functions act on $\r_+$ to $\r^{n,n}$, 
...
DAE systems of the forms \eqref{eq1}, \eqref{eq2} arise in many applications, include multibody dynamics, electrical circuit, chemical engineering, and many other applications. Due to the rank-deficiency of $E(t)$, the qualitative behavior of DAEs is much richer, in comparison to ordinary differential equations (ODEs). We refer the interested readers to recent monographs \cite{BieCM12,KunM06,LamMT13,IlcR13} and the references therein. In particular, even though the stability analysis for DAEs have been intensively discussed (see the survey \cite{IlcR13}, Chapter 2), there are only a few papers on the spectral theory of DAEs and in particular, the exponential dichotomy for DAEs. We refer to \cite{LenM90} for the concept of exponential dichotomy and its relation to the well conditioning of the associated boundary value problem, to \cite{LinM09} for Lyapunov and other spectra for linear DAEs, to \cite{ChyDL08,DuL06} for the robustness of exponential stability and Bohl exponents. 
Besides that, whenever the exponential dichotomy of the linear, homogeneous system \eqref{eq2} is characterized, the next important question in the qualitative theory of DAEs is to study whether integral manifolds (e.g., stable, unstable, center, center-stable, center-unstable) for the semi-linear DAE \eqref{eq1} exist, \cite{DalK02,Chi13}. Unfortunately, till now this question is essentially open for DAEs. In order to shorten these gaps, this paper is devoted to the exponential stability of \eqref{eq2} and stable manifolds of \eqref{eq1}. Our method is based on the classical "Lyapunov-Perron method" (\cite{DalK02,Zhi72}) and the admissibility of function spaces (\cite{Huy06,Huy09a}).

The outline of this paper is as follows. In the first section we recall some basis concepts which will be useful later, including an exponential dichotomy and its properties. Then, in Section \ref{Sec 2} we present the admissibility of function spaces and their properties. In Section \ref{Sec 3} we gives a characterization for the existence of exponential dichotomy for the DAE \eqref{eq2}. 
...
Now let us recall some basic notions. By ($\r^n$, $\n{\cdot}$) we denote the n-dimensional real vector space equipped with the Euclidean norm. For any matrix $V$, by $V^T$ we denote its transpose. \ 
For any $p \in \N$, by $C^p([0,\infty),\r^n)$ we denote the space of $p$-times continuously differentiable functions act on $[0,\infty)$ to $\r^n$. By $C_{\infty}([0,\infty),\r^n)$ we denote the space of continuous, bounded functions act on $[0,\infty)$ to $\r^n$. 
...
\end{align}
%
is well-posed, then there exists a pointwise nonsingular matrix-valued function $X(t,s)\in \r^{n,n}$ such that the solution of $\eqref{eq3}$ is given by  $x(t)=X(t,s)x_s$.
...
%\ee
%
is given by the so-called \emph{variational of constant formula}
%
\be\label{variational form}
x(t) = X(t,s)x(x) + \int_s^t X(t,\tau) f(\tau,x(\tau)) d\tau, \ \mbox{ for all }t\geq s \geq 0.
\ee
%
...
For more details on the notion and some problems focus on properties and applications of evolution families we refer the readers to Pazy, \cite{Paz12}.
%
...
Consider the DAE \eqref{eq2} and assume that it satisfies Assumption \ref{Ass1}. Then, there exists  pointwise-orthogonal matrix-valued functions $U$ and $V\in C^1([0,\infty),\r^{n,n})$, such that by changing variable $x(t) = V(t)y(t)$, and scaling \eqref{eq2} with $U(t)$, we can transform it
to the so-called \emph{decoupled system} of the following form
%
...
%	  
where $\Si(t)$ is continuous, pointwise nonsingular, matrix-valued function in $\r^{d,d}$. Changing the variable $x(t) = V(t)y(t)$ and scaling \eqref{eq2} with $U(t):=\m{U_1(t) & 0 \\ 0 & I_{a}}$, we obtain a new system 
%
\[
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%  Section 2%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Function Spaces and Admissibility} \label{Sec 2}

In this section we recall some notions of function spaces that play a fundamental role in the study of differential equations and refer to 
Massera and Sch\"affer \cite[Chap. 2]{Mas66} and R\"abiger and Schnaubelt \cite[\S 1]{RabS96} for their concrete applications.
...
We then define the Banach space corresponding to the space $E$ as follows.
\begin{definition}\label{def curly E}
Consider the Banach space $(\r{^n},\|\cdot\|)$ with some arbitrary norm. 
...
\]
%
(modullo $\lambda-$nullfunctions) endowed with the norm $\|f\|_\CE:=\|\|f(\cdot)\|\|_E.$
...
\begin{definition}\label{defE1}
	...
	\begin{enumerate}
		\item There exists a constant $M_{\vphi} \ge 1$ such that
		for every compact interval $ [a,b]\in \r_+$ we have
		\begin{equation}\label{embed}
		\int_a^b|\varphi(t)|dt\le 
		\frac{M_{\vphi}(b-a)}{\|\chi_{[a,b]}\|_E}\|\varphi\|_E\hbox{ for all }
		\varphi\in E.
		\end{equation}
...
\begin{example}{\rm Besides the spaces
		...
		function spaces over $(\r_+,\b,\lambda)$ (see \cite{Lin13}) are admissible.
	}
\end{example}

\begin{remark}\label{2.4}
Following directly from Definitions \ref{defE} ii) and \ref{defE1} i) we have that			
%
\[		
\underset{t\ge 0}{\sup}\int_t^{t+1}|\varphi(\tau)|d\tau\le 
\frac{M_{\vphi}}{\inf_{t\ge 0}\|\chi_{[t,t+1]}\|_E} \|\varphi\|_E,
\]
%
and hence, $E\hookrightarrow {\mathbf M}_1(\r_+)$. Furthermore, $C_{\infty}(\r^+)$ is dense in ${\mathbf M}_1$.
\end{remark}
	
We now collect some properties of 
%One of interesting properties of
admissible Banach function spaces in the following proposition (see \cite[Proposition 2.6]{Huy06}  and originally in \cite[23.V.(1)]{Mas66}).
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%  Section 3%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Exponential Dichotomy for linear DAEs and its properties}\label{Sec 3}

In the qualitative analysis of ODEs, one of the central topic is to find sufficient and necessary conditions for the considered system to admit exponential dichotomy. Many researches have been devoted to this topic, and critical results have been achieved for ODEs of both finite and infinite dimensions (e.g. \cite[Chap. 4]{DalK02}, \cite{Zhi72}). For DAEs, the only result that we are aware of is recalled below.
...
Notice that, even for ODEs, Proposition \ref{prop1} is only valid for finite but not infinite dimensional systems. 
For this reason, we recall another important result below.
%
\begin{proposition}\label{prop2} (\cite{DalK02}) Consider the ODE \eqref{eq3}. Then it has exponential dichotomy if and only if for any continuous, bounded function $f(t)$ on $[0,\infty)$, there exists a continuous, bounded solution $x(t)$.
\end{proposition}
%
...
\begin{definition}\label{EA-bounded}
	Consider the matrix functions $E$, $A$ in system \eqref{eq2}. Then, any function $f:\r_+ \rar \r^n$ satisfies the condition
	%
	\[
	\underset{t\geq 0}{\sup} \bigg\{ \bigg\| \m{\Si^{-1}(t) & -\Si^{-1}(t) \tA_2(t) \tA^{-1}_4(t) \\ 0 & \tA^{-1}_4(t)} f(t) \bigg\| \bigg\} < +\infty,
	\]
	%
	is called $(E,A)$-bounded. We denote the set of all continuous, $(E,A)$-bounded function by $\BEA$.
\end{definition}

The main result of this section is to prove, that "roughly speaking" the DAE \eqref{eq2} admits exponential dichotomy if and only if the mapping $\cL := E\dfrac{d}{dt} - A$ is surjective on the space $\BEA$. In connection with the solvability of the linear, inhomogeneous DAE
%
...
\end{equation}
%
we reform our main result in this section as follows.
\begin{theorem}\label{thm3.1}
	...
	i) If the DAE \eqref{eq2} admits exponential dichotomy then for any continuous, $(E,A)$-bounded function $f(t)$ on $[0,\infty)$, there exists a continuous, bounded solution $x(t)$ to the DAE \eqref{eq3.1}. \\
	ii) If the matrix function $\hA_3(t)$ is bounded, then the converse of i) is also true.
\end{theorem}

\begin{proof}
	First we notice that, since $\hf = U(t) \m{\Sigma^{-1}(t) & -\Si^{-1}(t) \tA_2(t) \tA^{-1}_4(t) \\ 0 & -\hA_4^{-1}} f(t)$, the $(E,A)$-boundedness of $f$ is equivalent to the boundedness of $\hf$. Recall that the decoupled system \eqref{decoupled system} reads
	%
	...
	%
	ii) Due to Proposition \ref{prop2}, it follows that \eqref{eq3.10a} has exponential dichotomy. 
	Besides that, due to the boundedness of $\hA_3$, it follows that \eqref{eq2} admits exponential dichotomy.
\end{proof}
...
Due to Lemma \ref{lem1}, by using orthogonal transformation $x(t)=V(t)y(t)$, where $y(t)= \m{y_1(t) \\ y_2(t)}  \in \r^{d+a}$ 
...
\end{equation}
%
Notice that, different from the DAEs \eqref{eq2} and \eqref{eq3.1}, equation \eqref{eq4.1b} only gives an implicit 
algebraic constraint in terms of $y_1$ and $y_2$. In order to guarantee the strangeness-free of system \eqref{eq1}, we need the following assumption. 

\begin{assumption}\label{Ass4.1}
Assume that for some $\rho > 0$, the function $A^{-1}_4(t) f_2(t,x)$ is a contraction mapping in the ball $B_{\rho}$ (uniformly in time), i.e., 
%
\[
...
Under Assumption \ref{Ass4.1}, restricted to the ball $B_{\rho} \subset \r{^n}$, $y_2$ can be uniquely solvable from \eqref{eq4.1b} in terms of $t$ and $y_1$.
\end{lemma}
%
\begin{proof}
First notice that Assumption \ref{Ass4.1} implies that $\hf_2(t,y)$ is also Lipschitz in $y$ with the same constant $L$. Then, the desired claim is obtained directly by making use of \cite[Lem. 2.7]{MinW04}.
\end{proof}

\begin{remark}\label{rem4.1}
Lemma \ref{lem4.0} leads to one critical fact, that under Assumption \ref{Ass4.1}, the coupled system \eqref{eq4.1a}-\eqref{eq4.1b} is still strangeness-free, as defined in \cite[Chap. 4]{KunM06}. Therefore, in analoguous to the linear case, \eqref{eq4.1b} is called \emph{an algebraic constraint}, while \eqref{eq4.1a} is called \emph{an underlying ODE}.
\end{remark}

To obtain the stable manifold we need the following property of the nonlinear part $f_1$ as be shown in the notion below.
%
\begin{definition}\label{def4.1}
	Let $E$ be an admissible Banach function space and $\vphi$ be a positive function belongs to $E$. A function $h:\hro{R_+}\times \r^{d}\rar\r^{d} $ is said to belong to the class $(M,\vphi,\rho)$ for some positive constant $M$, $\rho$ if $h$ satisfies
	\begin{enumerate}
...
Assume that the function \ $\Si^{-1}(t)  \ f_1(t,x(t)) - \Si^{-1}(t) \tA_2(t) \tA^{-1}_4(t)  \ f_2(t,x(t))$ \ belongs to the class $(M,\vphi,\rho)$ for some positive constants $M$, $\rho$ and a positive function $\vphi \in E$.
\end{assumption}
...
For notational simplicity, we will study the existence of a local stable manifold for system \eqref{eq4.1a}-\eqref{eq4.1b}. 
...
Let  $(\hY(t,s))_{t\geq s\geq 0}$ has an exponential dichotomy with the corresponding projection matrices $\{\Py(t)\}_{t\geq 0}$  and the dichotomy constants $N,\nu>0$ as in Definition \ref{def dichotomy}. Furthermore, due to Proposition \ref{prop1}, let us denote by $H_1 := \underset{t\geq 0}{\sup \ } \|\hA_3(t)\|$ and $H_2:= \underset{t\geq 0}{\sup \ } \|\Py(t)\|$. Then, we can define the Green function on the half-line as follows
...
Let the evolution family  $(\hY(t,s))_{t\geq s\geq 0}$ of system \eqref{decoupled system} has an exponential dichotomy with the corresponding projection matrices $\{\Py(t)\}_{t\geq 0}$  and the dichotomy constants $N,\nu>0$. Furthermore, assume that Assumptions \ref{Ass4.1}, \ref{Ass4.2} hold true. 
...
\end{lemma}
\begin{proof}
Put $z(t)=\m{z_1(t) \\ z_2(t)} := \int_{t_0}^{\infty} G(t,\tau) \m{\hf_1(\tau,y(\tau)) \\ 0} d\tau + \m{0 \\ \hf_2(t,y(t))}$, by direct computation, we can verify that $z$ satisfies the integral equation
%
\[
...
\]
and then, \eqref{eq2.1} follows that
%
	\[
...
\]
%
we see that $v_0:= z_1(t_0) - y_1(t_0) \in \im \Py(t_0)$. Finally, since $z(t)=y(t) + \hY(t,t_0) \m{v_0 \\ 0}$ for all $t\geq t_0$, \eqref{4.4} follows.
\end{proof}
...
\begin{lemma}\label{lem4.4}
	Under the assumptions of Lemma \ref{lem4.3}, let $y(t)$, $\ty(t)$ be any two functions lie in the ball $B_{\rho}$ and satisfy \eqref{4.4} for $v_0$, $\tv_0 \in \im \Py(t_0)$. If $H_3$ defined by \eqref{4.8} satisfies $H_3+L<1$ then we have the following estimation
	%
...
\end{equation}
%
is fulfilled, then there corresponds to each  $v_0\in B_{\trho} \cap \im \Py(t_0)$ one and only one solution $y(t)$ to $\eqref{mild equation}$ on $[t_0,\infty)$ satisfying $\Py(t_0)y_1(t_0)= v_0$ and $ess\sup_{t\geq t_0}\|y(t)\| \leq \rho$. \\
ii) Moreover, for any two solutions $y(t)$, $\ty(t)$ corresponding to different $v_0$, $\tv_0$ in $ B_{\trho} \cap \im \Py(t_0)$, they are attracted to each other exponentially, i.e.,
%
...
\eens
%
and due to \eqref{eq4.7} we see that
%
\[ \n{(Ty)(t)} \leq (1+H_1)N \trho \ +  \dfrac{(1-L) \rho}{2} + L \rho = \rho \ .
\]
%
...
%\ee
%%
Making use of \eqref{4.3}, we obtain the following estimation
%
\begin{align*}
...
\]
%
Then, due to the Cone Inequality, \cite[Theorem 1.9.3]{DalK02}, in analoguous to \cite[Theorem 3.7]{HUY09b}, we obtain the estimation \eqref{eq4.8} with $H_4$, $\mu$ are given by
%
\begin{align*}
...
\end{align*}
%
Furthermore, notice that due to \eqref{eq4.7} we see that $\mu<\nu$, which implies the positivity of $H_4$. This completes the proof.
\end{proof}
...
Under Assumption \ref{Ass4.1}, we define the so-called \emph{constrained manifold}, which all solutions to \eqref{eq4.1a}-\eqref{eq4.1b} must lie on
%
\be\label{contraint manifold}
\bbL(t,y):= \{(t,y_1,y_2) \in \r_+ \times \r^d \times \r^a\ | \  y_2 = \hA_3(t)y_1 + \hf_2(t,y_1,y_2) \}  \ . 
\ee
%
We further notice that this manifold is of dimension $d$, which is the degree of freedom to the DAE \eqref{mild equation}. Now we are able to introduce the concept of a local stable manifold for the solutions of the integral-algebraic system \eqref{mild equation}.
...
Let the evolution family  $(\hY(t,s))_{t\geq s\geq 0}$ of system \eqref{decoupled system} have an exponential dichotomy with the corresponding projection matrices $\{\Py(t)\}_{t\geq 0}$  and the dichotomy constants $N,\nu>0$. Furthermore, assume that Assumptions \ref{Ass4.1}, \ref{Ass4.2} hold true. If the condition 
%
...
\end{equation*}
%
is fulfilled, then there exists a local stable manifold for the solutions of $\eqref{mild equation}$. Moreover, every two solutions $y(t)$, $\ty(t)$ on the manifold $\bbM$ attract each other exponentially in the sense that there exist positive constants $H_4$ and $\mu$ independent of $t_0 \geq 0$ such that 
%
...
For any $\rho>0$ corresponding to Assumptions \ref{Ass4.1}, \ref{Ass4.2}, let $\rho_1 := \trho = \dfrac{1-L}{2N(1+H_1)} \rho$ \ and \ $\rho_2 := \dfrac{(1-L)\rho}{2}$. For each $t \geq 0$ we define the mapping 
...
Now we prove that $\n{g_{t}(w_1)} \leq \rho_2$. Due to Theorem \ref{thm4.1} i) and Lemma \ref{lem4.1} i), we see that $\n{y(t)} \leq \rho$ and $\n{f_1(\tau,y(\tau))} \leq M \vphi(\tau)$ for a.e. $t\geq 0$. Therefore,
%
\bens
...
\ $Id+g_t: \bbM_t \rar B_{\rho_1} \cap W_1(t)$ \ is a homeomorphism. This implies the condition ii) of Definition \ref{local stable manifold}, and hence, the proof is finished. 
\end{proof}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Global stable manifolds for semi-linear DAEs}\label{Sec 5}
...
Let $E$ be an admissible Banach function space and $\vphi \in E$ is a positive function. A function $h:\hro{R}_+ \times \r^{n}\rar\r^{d} $ is said to be \emph{$\vphi$-Lipschitz} if the following conditions hold true.
	\begin{enumerate}
...
In comparable to Assumptions \ref{Ass4.1}, \ref{Ass4.2}, we also need some global properties of the nonlinear term $f$.

\begin{assumption}\label{Ass5.1}
Assume that the followings hold true.\\
i) The function  \ $\Si^{-1}(t)  \ f_1(t,x(t)) - \Si^{-1}(t) \tA_2(t) \tA^{-1}_4(t)  \ f_2(t,x(t))$ \ is $\vphi$-Lipschitz. \\
ii) The function $\tA^{-1}_4(t) \ f_2(t,x(t))$ is a contraction mapping with the Lipschitz constant $L<1$ for all $(t,x(t))$ lies on the constraint-manifold associated with \eqref{eq1} defined by
\begin{equation*}
\bbL(t,x):= \{(t,x) \in \r_+ \times \r^n \ | \  0 = A_2(t)x + f_2(t,x) \}  \ . 
\end{equation*}
\end{assumption}
...
\begin{definition}\label{global stable manifold}
	A subset $\bbM$ of the constrained manifold $\bbL(t,y)$ is said to be a \emph{global, stable manifold} for solutions to $\eqref{mild equation}$  if for every $t\in\hro{R_+}$  the phase subspace $\r^{d}$ splits into a direct sum  $\r^{d} = W_1(t)\oplus  W_2(t)$  such that
	\begin{equation*}
	\inf_{t\in\hro{R_+}}Sn(W_1(t), W_2(t)):=\inf_{t\in\hro{R_+}} \ \inf\{\|w_1+w_2\|, \  w_i\in W_i(t),\  \|w_i\| = 1, \ i=0,1 \} >0,
	\end{equation*}
	and if there exists a family of Lipschitz continuous mappings
	...
	with the Lipschitz constants independent of  $t$ such that
	\begin{enumerate}
		\item[(i)]$\bbM=\{(t,y_1 = w_1 + g_t(w_1),y_2)\in\hro{R_+} \times (W_1(t)\oplus W_2(t)) \times \r^a \mid w_1 \in W_1(t)\}$, and we denote by 
		$\bbM_t:=\{ (y_1 = w_1 + g_t(w_1),y_2) \mid (t,y_1 = w_1+g_t(w_1),y_2) \in \bbM \}$,
...
In analogous to Lemma \ref{lem4.3}, we give the explicit form of bounded solutions to system \eqref{mild equation} as below.
%
\begin{lemma}\label{lem5.1}
	Let the evolution family  $(\hY(t,s))_{t\geq s\geq 0}$ of system \eqref{decoupled system} has an exponential dichotomy with the corresponding projection matrices $\{\Py(t)\}_{t\geq 0}$  and the dichotomy constants $N,\nu>0$. Furthermore, assume that Assumption \ref{Ass5.1} holds true. 
	Let $y(t)$ be any solution to \eqref{mild equation} such that $ess \sup_{t\geq t_0} \|y(t)\| < \infty$ for fixed $t_0 \geq 0$. 
...
\end{lemma}
\begin{proof}
The proof can be achieved by using similar arguments as done in the proof of Lemma \ref{lem4.0}, and we will omit the details here in order to keep the brevity of this research.	
\end{proof}
...
\begin{theorem}\label{thm5.1}
	Let the evolution family  $(\hY(t,s))_{t\geq s\geq 0}$ of system \eqref{decoupled system} have an exponential dichotomy with the corresponding projection matrices $\{\Py(t)\}_{t\geq 0}$  and the dichotomy constants $N,\nu>0$. Furthermore, assume that Assumption \ref{Ass5.1} holds true. \\
	i) For any fixed $t_0 \geq 0$, if the condition 
	%
	\begin{equation*}
	%\dfrac{H_3}{1-e^{-\nu}} 
	H_3 <  1-L
	\end{equation*}
	%
	is fulfilled, then there corresponds to each  $v_0\in \im \Py(t_0)$ one and only one solution $y(t)$ to $\eqref{mild equation}$ on $[t_0,\infty)$ satisfying $\Py(t_0)y_1(t_0)= v_0$ and $ess\sup_{t\geq t_0}\|y(t)\| < \infty$. \\
	ii) Moreover, for any two solutions $y(t)$, $\ty(t)$ corresponding to different $v_0$, $\tv_0$ in $\im \Py(t_0)$, they are attracted to each other exponentially, i.e.,
	%
	\begin{equation*}
	\n{y(t) - \ty(t)} \leq H_4 \ e^{-\mu (t-t_0)} \ \n{v_0-\tv_0}, \ \mbox{ for all } t\geq t_0,
	\end{equation*}
	%
	for some positive constants $H_4$, $\mu$ satisfying
	%
	\begin{align*}
	0 < \mu < \nu + \ln \left(1- \dfrac{H_3(1-e^{-\nu})}{1-L}\right), \quad 
	H_4 := \dfrac{(1+H_1)N}{1-L-\dfrac{H_3(1-e^{-\nu})}{1-e^{\mu-\nu}}} \ .
...
\begin{theorem}\label{thm5.2}
	Let the evolution family  $(\hY(t,s))_{t\geq s\geq 0}$ of system \eqref{decoupled system} have an exponential dichotomy with the corresponding projection matrices $\{\Py(t)\}_{t\geq 0}$  and the dichotomy constants $N,\nu>0$. Furthermore, assume that Assumption \ref{Ass5.1} holds true. If the condition 
	%
	\begin{equation*}
	%\dfrac{H_3}{1-e^{-\nu}} 
	...
	\end{equation*}
	%
	is fulfilled, then there exists a global stable manifold for the solutions of $\eqref{mild equation}$. Moreover, every two solutions $y(t)$, $\ty(t)$ on the manifold $\bbM$ attract each other exponentially in the sense that there exist positive constants $H_4$ and $\mu$ independent of $t_0 \geq 0$ such that 
	%
	\begin{equation*}
	\n{y(t) - \ty(t)} \leq H_4 \ e^{-\mu (t-t_0)} \ \n{P(t_0) y_1(t_0) - P(t_0) y_2(t_0)}, \ \mbox{ for all } t\geq t_0 \ 	.
	\end{equation*}
...
\end{align*}
%
will have one positive and one negative eigenvalue (e.g. \cite{Ria04}). Hence, it admits exponential dichotomy for any odd number $n$. 
...
The second author was supported the National Foundation for Science and Technology Development (NAFOSTED) under the project number 101.01-2017.302. He also would like to thank the Vietnam Institute for Advanced Study in Mathematics (VIASM) for their kind hospitality during his research visit.
