...
\]
%
and hence, this follows that $S_3 = 0$. Thus, $S=\sm{S_1 & S_2 \\ 0 & S_4}$, and then, due to the orthogonality of $S$, we see that both $S_1$ is nonsingular and $S_4$ is also orthogonal. Also from \eqref{15.1}, we see that $S_1\Si T_2 = 0$, which yields that $T_2 = 0$. Moreover, due to the orthogonality of $S$ and $T$, from \eqref{eq14.1} we see that 
%
\[
...
\end{subequations}
%
where the matrix-valued function $S_i$, $T_i$ ($i=1$, $4$) are pointwise orthogonal. Thus, we see that 
%
...
Since the Euclidean norm is preserved under orthogonal transformations, due to \eqref{eq11}-\eqref{eq7} we see that 
%
\begin{equation*}
...
Consider the system \eqref{eq2.1} with $E=\sm{1 & 0 \\ 0 & 0}$, $A=\sm{-1 & 0 \\ 0 & e^{-t}}$, $f=\sm{0 \\ 1}$. Clearly, $f$ is bounded. The explicit solution
$x(t) = \sm{e^{-t}x_1(0) \\ e^{t}}$ is clearly unbounded no matter how the initial condition $x(0)$ is chosen.  On the other hand, the homogeneous system clearly has an exponential dichotomy.
\end{example}
...
\begin{lemma}
The $(E,A)$-boundedness is invariant with respect to orthogonal transformations.  
\end{lemma}
\begin{proof}
...
%
\begin{align*}
\m{\tSi^{-1}(t) & -\tSi^{-1}(t) \tA_2(t) \tA^{-1}_4(t) \\ 0 & \tA^{-1}_4(t)} g(t) = 
\m{T^T_1 & 0 \\ 0 & T^T_4} \m{\Si^{-1}(t) & -\Si^{-1}(t) A_2(t) A^{-1}_4(t) \\ 0 & A^{-1}_4(t)} \m{S^T_1 & 0 \\ 0 & S^T_4} g(t) \ .
\end{align*}
%
...
\begin{theorem}\label{thm3.1}
	...
	\begin{enumerate}[(i)]
	\item If the DAE \eqref{linDAE} admits an exponential dichotomy then for any continuous, $(E,A)$-bounded function $g(t)$ on $[0,\infty)$, there exists a continuous, bounded solution $x(t)$ to the DAE \eqref{eq2.1}. 
	\item If the matrix function $\hA_3(t)$ is bounded, then the converse of assertion (i) holds true.
	\end{enumerate}
\end{theorem}

\begin{proof}
	Firstly, we notice that, since $\hg = \m{\Sigma^{-1}(t) & -\Si^{-1}(t) A_2(t) A^{-1}_4(t) \\ 0 & -\hA_4^{-1}} U(t) g(t)$, where $U$ is orthogonal, so the $(E,A)$-boundedness of $g$ is equivalent to the boundedness of $\hg$. Recall that the semi-explicit system \eqref{semi-explicit system} reads
	%
	...
	%
	\[
		\m{\Sigma^{-1}(t) & -\Si^{-1}(t) A_2(t) A^{-1}_4(t) \\ 0 & -\hA_4^{-1}} U(t) \ : \BEA \rightarrow C_{\infty}(\r_+,\r^n)
	\]
	%
	is surjective, so $g_1(t)$ can be freely chosen in the space $C_{\infty}(\r_+,\r^n)$. Proposition \ref{prop2} applied to system \eqref{eq3.10a} follows that \eqref{eq3.10a} has exponential dichotomy. 
...
The following lemma shows that Assumptions \ref{Ass4.1}, \ref{Ass4.2} are invariant with respect to orthogonal transformations. 
%
\begin{lemma}\label{lem4.2}
Assumptions \ref{Ass4.1}, \ref{Ass4.2} are also invariant with respect to orthogonal transformations. 
\end{lemma}
%
\begin{proof}
Let us consider two orthogonally equivalent systems
...
= \n{T_4^{-1}(t)  A^{-1}_4(t)  S_4^{-1}(t) \left( f_2(t,x) - f_2(t,\tx) \right) } 
\leq L \|x-\tx \|, 
\end{align*}%
Then, the desired claim is directly followed.
\end{proof}
