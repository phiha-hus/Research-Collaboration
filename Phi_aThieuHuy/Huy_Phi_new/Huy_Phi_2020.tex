...
pointwise-orthogonal matrix-valued functions $U\in C^0([0,\infty),\r^{n,n})$ and $V\in C^1([0,\infty),\r^{n,n})$, such that after changing variable $x(t) = V(t)y(t)$, and scaling \eqref{linDAE} with $U(t)$, we obtain exactly \eqref{eq7}. In details, this means that the following identities hold true.
%
\begin{equation}
...
%	Consider the DAE \eqref{semi linDAE} and assume that it satisfies Assumption \ref{Ass1}. \\
...
\begin{alignat}{2}
 \dot{y}_1(t) &= \hA_1(t) y_1(t), \label{eq10.1} \\ 
       y_2(t) &= \hA_3(t) y_1(t). \label{eq10.2}
\end{alignat}
\end{subequations}
%
Since $V(t)$ is orthogonal for all $t\geq 0$, we see that all important qualitative properties of $x(t)$, such as boundedness, exponential stability, contractivity, expansiveness, etc., can be carried out for the function $y(t)$ without any difficulty. Clearly, we see that \eqref{eq10.2} gives an \emph{algebraic constraint} that the solution to \eqref{semi-explicit system} must obey, while \eqref{eq10.1} gives the dynamic of \eqref{semi-explicit system}. 
...
Let $\{\hY_1(t,s)\}_{t\geq s \geq 0}$ be the evolution family associated with \eqref{eq10.1}, then we can define the corresponding evolution families for two DAEs \eqref{semi-explicit system}, \eqref{linDAE} consecutively as follows.
%
...
Therefore, the underlying ODE \eqref{eq14.1} is directly obtained from \eqref{eq10.1} by applying the variable transformation $\ty_1(t)=T_1(t) y_1(t)$ and scaling the system with $T^{-1}_1$. So, we have that $\hcY_1(t,s) = T_1^{-1} \hY_1(t,s) T_1(s)$. 
...
Since the Euclidean norm is preserved under orthogonal transformations, due to \eqref{eq11} and \eqref{eq19} we have
%
\begin{equation*}
...
The DAE \eqref{semi-explicit system} has an exponential dichotomy if and only if the corresponding underlying ODE \eqref{eq10.1} also has exponential dichotomy and the matrix function $\hA_3(t)$ is bounded. Moreover, the existence of an exponential dichotomy implies that $\underset{t\geq 0}{\sup \ } \|\Py(t)\| < \infty$. 
\end{proposition}
...
%
\begin{equation}\label{eq2.2}
C_b^{sys}(\r_+):=\left\{g \in C(\r_+) \ \Bigg| \ \underset{t\geq 0}{\sup} \bigg\{ \bigg\| \m{\Si^{-1}(t) & -\Si^{-1}(t) A_2(t) A^{-1}_4(t) \\ 0 & A^{-1}_4(t)} U(t) g(t) \bigg\| \bigg\} < +\infty \right\} \ ,
\end{equation}
%
...
\end{align*}
%
where $(E,A,g) \osim (\tE,\tA,\tg)$. Without loss of generality, in analogous to Lemma \ref{lem4}, we may assume that 
%
\[
(E,A) = \left( \m{\Si & 0 \\ 0 & 0},\m{A_1 & A_2 \\ A_3 & A_4} \right), \mbox{ and } (\tE,\tA) = \left( \m{\tSi & 0 \\ 0 & 0},\m{\tA_1 & \tA_2 \\ \tA_3 & \tA_4} \right) \ .
\]
%
Under this assumption, $U(t)=\tU(t)=I_n$ for all $t\geq 0$. As shown in the proof of Lemma \ref{lem4}, the identities \eqref{eq17} hold true for some pointwise orthogonal matrix-valued functions $S_i$, $T_i$ ($i=1$, $4$). 
Thus, we see that
%
\begin{align*}
...
The main result of this section is to prove a characterization of the exponential dichotomy for DAEs. Roughly speaking, the DAE \eqref{linDAE} admits exponential dichotomy if and only if the mapping $\cL := E\dfrac{d}{dt} - A$ is surjective on the space $\BEA$. 
...
\begin{proof}
	Firstly, we notice that, since $\hg(t) = \m{\Sigma^{-1}(t) & -\Si^{-1}(t) A_2(t) A^{-1}_4(t) \\ 0 & -\hA_4^{-1}} U(t) g(t)$, so the fact $g \in \Csys$ implies the boundedness of $\hg$. Recall that the semi-explicit system \eqref{semi-explicit system} reads
	%
	...
	%
	\[
		\m{\Sigma^{-1}(t) & -\Si^{-1}(t) A_2(t) A^{-1}_4(t) \\ 0 & -\hA_4^{-1}} U(t) \ : \BEA \rightarrow C_{b}(\r_+,\r^n)
	\]
	%
	is surjective, so $g_1(t)$ can be freely chosen in the space $C_{b}(\r_+,\r^n)$. Proposition \ref{prop2} applied to system \eqref{eq3.10a} follows that \eqref{eq3.10a} has exponential dichotomy. 
	On the other hand, the boundedness of $\hA_3$ implies that \eqref{linDAE} admits exponential dichotomy.
\end{proof}
...
Throughout this section we assume that the evolution family  $(X(t,s))_{t\geq s\geq 0}$ associated with the linear, homogeneous DAE \eqref{linDAE} admits an exponential dichotomy on $\hro{R_+}$. 

From Lemma \ref{lem1}, by using orthogonal transformation $x(t)=V(t)y(t)$, where $y(t)= \sm{y_1(t) \\ y_2(t)}  \in \r^{d+a}$ 
we can transform \eqref{semi linDAE} to the coupled system
%
\begin{align}
...
\m{\Sigma^{-1}(t) & -\Si^{-1}(t) A_2(t) A^{-1}_4(t) \\ 0 & -\hA_4^{-1}} U(t) \m{f_1(t,x(t)) \\ f_2(t,x(t))} \ .
\end{equation}
%
Notice that, unlike the DAEs \eqref{linDAE} and \eqref{eq2.1}, equation \eqref{eq4.1b} only gives an implicit algebraic constraint in terms of $y_1$ and $y_2$. In order to guarantee the strangeness-free of system \eqref{semi linDAE}, we need the following assumption. 
...
Under Assumption \ref{Ass4.1} and given $y_1\in B_{\rho}$, there exists a unique function
 $y_2\in \B_{\rho}$ satisfying  \eqref{eq4.1b}.
...
The following lemma shows that Assumptions \ref{Ass4.1}, \ref{Ass4.2} are invariant with respect to system orthogonal transformations. 
%
\begin{lemma}\label{lem4.2}
Assumptions \ref{Ass4.1}, \ref{Ass4.2} are also invariant with respect to system orthogonal transformations. 
\end{lemma}
%
\begin{proof}
...
\leq L \|x-\tx \|, 
\end{align*}%
Then, due to the orthogonality of $T_4$, the desired claim is directly followed.
\end{proof}

The following proposition gives one sufficient condition for examining Assumptions \ref{Ass4.1}, \ref{Ass4.2}.

\begin{proposition}
Consider the semi-linear DAE \eqref{semi linDAE}. Furthermore, assume that all three functions $\Si^{-1}$, $A^{-1}_4$, $\Si^{-1} A_2 A^{-1}_4$ are bounded. If the function $f = \sm{f_1 \\ f_2}$ belongs to the class $(M,\vphi,\rho)$ then the following claims hold true. \\
i) $\hf_1$ belongs to the class $(M,\vphi,\rho)$, and \\
ii) $f_2$ is Lipschitz with the Lipschitz constant $\vphi \ \underset{t\geq 0}{\sup} \ \n{A^{-1}_4}$.

We notice that a sufficient condition for Assumption \eqref{Ass4.1} is that 
%
...
of system \eqref{semi-explicit system} have an exponential dichotomy with the corresponding projection matrices $\{\Py(t)\}_{t\geq 0}$  and the dichotomy constants $N,\nu>0$. Furthermore, assume that Assumptions \ref{Ass4.1}, \ref{Ass4.2} hold true, and constant $H_3$ defined as in \eqref{4.8}. Then, the following assertions hold true.
...
For any $\rho>0$ defined as in  Assumptions \ref{Ass4.1}, \ref{Ass4.2}, let $\rho_1 := \trho = \dfrac{1-L}{2N(1+H_1)} \rho$ \ and \ $\rho_2 := \dfrac{(1-L)\rho}{2}$. For each $t \geq 0$ we define the mapping $g_{t}$ acting on $B_{\rho_1} \cap W_1(t)$ as 
%
\[
...
\begin{example}\label{exam5.1}
		...
		\end{align}
		%
		Clearly, we still need to linearize \eqref{eq5.3} to obtain system of the form \eqref{semi linDAE}.
		Fortunately, in this case the linearization procedure around a trajectory yields the decoupled form \eqref{semi-explicit system}
		%
		\begin{align}\label{eq5.4}
		M \dot{U} &= A_1(t) U + A_2(t) P + g_1(t,U,P), \notag \\
		0 &= C^TM^{-1} \left( C - \dfrac{\partial F}{\partial P} \right) P - C^TM^{-1} \left(\dfrac{\partial F}{\partial U} - K \right) U +  C^TM^{-1} g_2(t,U,P) \ .
		\end{align}
		%
		...
		%
		\[
		A_3(t) :=  - C^TM^{-1} \left(\dfrac{\partial F}{\partial U} - K \right) , \ A_4(t) := C^TM^{-1} \left( C - \dfrac{\partial F}{\partial P} \right) \]
		%
		Consequently, if the homogenous DAE 
		%
		\[
		\m{M & 0 \\ 0 & 0} \m{\dot{U} \\ \dot{P} } = \m{A_1(t) & A_2(t) \\ A_3(t) & A_4(t)} \m{U \\ P}
		\]
		%
