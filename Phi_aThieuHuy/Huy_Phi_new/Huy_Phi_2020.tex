...
\end{equation}
%
Here $E=\m{\bfE_1(t) \\ 0}$, $A = \m{\bfA_1(t) \\ \bfA_2(t)}$ are assumed to be matrix-valued functions acting 
on $\r_+$ to $\r^{n,n}$, 
$x: \r_+  \rightarrow \r^{n}$, $f:\r_+ \times \r{^n} \rightarrow \r{^n}$. Furthermore, we assume that for all $t$, the matrices $\bfE_1(t)$, $\bfA_2(t)$ have full row rank. 
...
Therefore, throughout this paper, we will assume the following.
%
...
%%
By making use of some smooth factorizations, for example QR or SVD (\cite{DieE99}  or \cite[Thm 3.9]{KunM06}), we can decouple and then exploit the structure of the DAE \eqref{linDAE} in the following lemma.
...
%
\[
\m{\Si(t) & 0 \\ A_3(t) & A_4(t)} = \m{U_1(t) & 0 \\ 0 & I_{a}} \ \m{\bfE_1(t) \\ \bfA_2(t)} \ V,
\]
%
...
Let $\{\hY_1(t,s)\}_{t\geq s \geq 0}$ be the evolution family associated with the matrix function $\hA_1(t)$, then we can define the corresponding evolution families for two DAEs \eqref{semi-explicit system}, \eqref{linDAE} consecutively as follows.
%
...
The families $\{X(t,s)\}_{t\geq s\geq 0}$, $\{X^-(t,s)\}_{t\geq s\geq 0}$ defined by \eqref{eq4}, \eqref{eq5} do not depend on the choice of orthogonal transformations.
\end{lemma}
...
(E,A) \osim \left( \m{\Si & 0 \\ 0 & 0},\m{A_1 & A_2 \\ A_3 & A_4} \right)
\mbox{ and }
(E,A) \osim \left( \m{\tSi & 0 \\ 0 & 0},\m{\tA_1 & \tA_2 \\ \tA_3 & \tA_4} \right) \ .
\end{equation}
The transitivity implies that these two semi-explicit systems are also orthogonally equivalent. Thus, there exist pointwise-orthogonal matrix-valued functions $S\!=\!\sm{S_1 & S_2 \\ S_3 & S_4}\in \! C^0([0,\infty),\r^{n,n})$ and $T\!=\!\sm{T_1 & T_2 \\ T_3 & T_4}\in \! C^1([0,\infty),\r^{n,n})$, such that
\begin{eqnarray}
...
\end{eqnarray}
The first identity implies that $S_3 \Si \m{T_1 & T_2} = 0$. Notice that product of two full row rank matrices is also a full row rank matrix (see e.g. \cite{GolV96}), 
this follows that $S_3 = 0$. Thus, $S=\sm{S_1 & S_2 \\ 0 & S_4}$, and hence, due to the orthogonality of $S$, we see that both $S_1$ is nonsingular and $S_4$ is also orthogonal. Also from \eqref{14}, we see that $S_1\Si T_2 = 0$, and hence $T_2 = 0$. Consequently, by inserting $S_3=0$ and $T_2=0$ into \eqref{14} and \eqref{15} we obtain
%
...
\end{subequations}
%
Now we determine the underlying equation \eqref{eq10.1} and the algebraic constraint equation \eqref{eq10.2} for the second system in \eqref{eq6}.
...
\begin{proposition}\label{prop1} (\cite{LinM09})
	The DAE \eqref{semi-explicit system} has an exponential dichotomy if and only if the corresponding underlying ODE \eqref{eq10.1} also has exponential dichotomy, and the matrix function $\hA_3(t)$ is bounded. Moreover, the existence of exponential dichotomy imlies that $\underset{t\geq 0}{\sup \ } \|\Py(t)\| < \infty$. 
\end{proposition}
...
\end{equation}
%
Notice that, even for ODEs, Proposition \ref{prop1} is only valid for finite-dimensional systems but not for infinite dimensional systems, \cite{DalK02}. 
For this reason, we recall a classical result by Perron below.
%
\begin{proposition}\label{prop2} (\cite{DalK02}) The ODE \eqref{eq3} has an exponential dichotomy if and only if for any continuous, bounded function $g(t)$ on $[0,\infty)$, there exists a continuous, bounded solution $x(t)$ to the system
%
...
\end{proposition}
%
In view of Proposition \ref{prop2}, comparable result has not been achieved for DAEs, and hence, this will be our main aim in this section.
...
\begin{theorem}\label{thm3.1}
	Consider the linear, strangeness-free DAE \eqref{linDAE} and the associated inhomogeneous DAE \eqref{eq3.1}. 
	%Then, the DAE \eqref{linDAE} admits exponential dichotomy if and only if for any continuous, $(E,A)$-bounded function $g(t)$ on $[0,\infty)$, there exists a continuous, bounded solution $x(t)$ to the DAE \eqref{eq3.1}.
	Then the following assertions hold. 
	\begin{enumerate}[(i)]
	\item If the DAE \eqref{linDAE} admits exponential dichotomy then for any continuous, $(E,A)$-bounded function $g(t)$ on $[0,\infty)$, there exists a continuous, bounded solution $x(t)$ to the DAE \eqref{eq3.1}. 
	\item If the matrix function $\hA_3(t)$ is bounded, then the converse of assertion (i) holds true.
	\end{enumerate}
\end{theorem}

\begin{proof}
	Firstly, we notice that, since $\hg = \m{\Sigma^{-1}(t) & -\Si^{-1}(t) A_2(t) A^{-1}_4(t) \\ 0 & -\hA_4^{-1}} U(t)
	 g(t)$, the $(E,A)$-boundedness of $f$ is equivalent to the boundedness of $\hf$. Recall that the semi-explicit system \eqref{semi-explicit system} reads
	%
	...
	(i) Assuming that the DAE \eqref{linDAE} admits exponential dichotomy, then \eqref{semi-explicit system} also has an exponential dichotomy. Proposition \ref{prop1} implies that equation \eqref{eq3.10a} has an exponential dichotomy, and the function $\hA_3$ is bounded. Therefore, Proposition \ref{prop2} implies that $y_1$ is bounded, and consequently, $y_2$ is also bounded.   
	%

	(ii) From Proposition \ref{prop2}, it follows that \eqref{eq3.10a} has exponential dichotomy \textbf{May be wrong, since the domains of g and hg are not the same}.  On the other hand, the boundedness of $\hA_3$ implies that \eqref{linDAE} admits exponential dichotomy.
\end{proof}
...
\m{\Sigma^{-1}(t) & -\Si^{-1}(t) A_2(t) A^{-1}_4(t) \\ 0 & -\hA_4^{-1}} U(t) \m{f_1(t,y(t)) \\ f_2(t,y(t))} \ .
\end{equation}
%

%
\[
\hf = \m{\hf_1 \\ \hf_2} := \m{\Si^{-1}(t) & 0 \\ 0 & -A^{-1}_4(t) } U(t) f(t,x(t)) 
\]
%

Notice that, unlike the DAEs \eqref{linDAE} and \eqref{eq3.1}, equation \eqref{eq4.1b} only gives an implicit algebraic constraint in terms of $y_1$ and $y_2$. In order to guarantee the strangeness-free of system \eqref{semi linDAE}, we need the following assumption. 
...
\begin{proposition}
Consider the semi-linear DAE \eqref{semi linDAE}. Furthermore, assume that all three functions $\Si^{-1}$, $A^{-1}_4$, $\Si^{-1} A_2 A^{-1}_4$ are bounded. If the function $f = \m{f_1 \\ f_2}$ belongs to the class $(M,\vphi,\rho)$ then the following claims hold true. \\
