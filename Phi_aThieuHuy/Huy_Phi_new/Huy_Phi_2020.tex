...
The concept exponential dichotomy means that the state space $\r^n$ has been splitted into the stable subspace ($\im(P(t))$)
and the unstable subspace ($\ker(P(t))$). 
...
Assume that the function pair $(E,A)$ in the DAEs \eqref{eq1} and \eqref{eq2} is \emph{strangeness-free}, i.e., 
%
\[
...
%
\begin{remark}
In general, not all systems of the type or \eqref{eq2} is strangeness-free. Nevertheless, under some constant rank assumption, one can always transform it, without alternating the solution space, to the strangeness-free form. For further details, see \cite{KunM06,BreCP96,LamMT13}.
\end{remark}
%%
...
Consider the DAE \eqref{eq2} and assume that it satisfies Assumption \ref{Ass1}. Then, there exist  pointwise-orthogonal matrix-valued functions $U\in C^0([0,\infty),\r^{n,n})$ and $V\in C^1([0,\infty),\r^{n,n})$, such that after changing variable $x(t) = V(t)y(t)$, and scaling \eqref{eq2} with $U(t)$, we can transform it to the so-called \emph{semi-explicit system} of the following form
%
...
\]
%
which is exactly of the form \eqref{semi-explicit system}. Furthermore, notice that 
%
\[
...
\end{proof}
%
Let $\hA_3 := -\tA^{-1}_4 \tA_3$, $\hA_s := \tA_1 + \tA_2 \tA^{-1}_4 \tA_3$ is the Schur complement, we rewrite system \eqref{semi-explicit system} as 
%
\begin{align}
\label{eq5.1} \dot{y}_1(t) &= \Si^{-1}(t) \hA_s(t) y_1(t), \\ 
\label{eq5.2} y_2(t) &= \hA_3(t) y_1(t).
\end{align}
%
Since $V(t)$ is orthogonal for all $t\geq 0$, we see that all important qualitative properties of $x(t)$, such as boundedness, exponential stability, contractivity, expansiveness, etc., can be carried out for the function $y(t)$. Clearly, we see that \eqref{eq5.2} gives an \emph{algebraic constraint} that the solution to \eqref{semi-explicit system} must obey, while \eqref{eq5.1} gives the dynamic of \eqref{semi-explicit system}. 
For this reason, we call it \emph{an underlying ODE} to \eqref{semi-explicit system}.\\

Let $\{\hY_1(t,s)\}_{t\geq s \geq 0}$ be the evolution family associated with the matrix function $\hA_1(t)$, then we can define the corresponding evolution families for two DAEs \eqref{semi-explicit system}, \eqref{eq2} consecutively as follows.
%
...
\end{align*}
%
Furthermore, Lemma \ref{lem5} below shows that the family $\{\hX(t,s)\}$ play the same role as the evolution family $(X(t,s))_{t\geq s\geq 0}$, in comparison to \eqref{variational form}.

\begin{lemma}\label{lem5}
Consider the DAE \eqref{eq1} and the evolution family $(X(t,s))_{t\geq s\geq 0}$ defined by \eqref{eq4}. Furthermore, we also consider the pointwise-orthogonal matrix-valued functions $U$, $V$ defined in Lemma \ref{lem5}. Then, the solution to \eqref{eq1}, if exists, also satisfies the so-called \emph{mild equation}
	%
	\begin{align*}
...
\eqref{eq2} as the one of \eqref{semi-explicit system}. 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%  Section 2%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{proposition}\label{prop1} (\cite{LinM09})
	The DAE \eqref{semi-explicit system} has an exponential dichotomy if and only if the corresponding underlying ODE \eqref{eq5.1} also has exponential dichotomy, and the matrix function $\hA_3(t)$ is bounded. Moreover, the existence of exponential dichotomy imlies that $\underset{t\geq 0}{\sup \ } \|\Py(t)\| < \infty$. 
\end{proposition}

Together with \eqref{eq2}, let us consider the following system
%
...
\begin{definition}\label{EA-bounded}
	Consider the matrix functions $E$, $A$ in system \eqref{eq2}. Then, a function $g:\r_+ \rar \r^n$ satisfying the condition 
	%
	\[
...
The main result of this section is to prove a characterization of the exponential dichotomy for DAEs. Roughly speaking, the DAE \eqref{eq2} admits exponential dichotomy if and only if the mapping $\cL := E\dfrac{d}{dt} - A$ is surjective on the space $\BEA$. 
...
\begin{theorem}\label{thm3.1}
	...
	%Then, the DAE \eqref{eq2} admits exponential dichotomy if and only if for any continuous, $(E,A)$-bounded function $g(t)$ on $[0,\infty)$, there exists a continuous, bounded solution $x(t)$ to the DAE \eqref{eq3.1}.
	Then the following assertions hold. 
	\begin{enumerate}[(i)]
	\item If the DAE \eqref{eq2} admits exponential dichotomy then for any continuous, $(E,A)$-bounded function $g(t)$ on $[0,\infty)$, there exists a continuous, bounded solution $x(t)$ to the DAE \eqref{eq3.1}. 
...
\begin{proof}
	...
	(i) Assuming that the DAE \eqref{eq2} admits exponential dichotomy, then \eqref{semi-explicit system} also has an exponential dichotomy. Proposition \ref{prop1} implies that equation \eqref{eq3.10a} has an exponential dichotomy, and the function $\hA_3$ is bounded. Therefore, Proposition \ref{prop2} implies that $y_1$ is bounded, and consequently, $y_2$ is also bounded.   
	%

	(ii) From Proposition \ref{prop2}, it follows that \eqref{eq3.10a} has exponential dichotomy \textbf{May be wrong, since the domains of g and hg are not the same}.  On the other hand, the boundedness of $\hA_3$ implies that \eqref{eq2} admits exponential dichotomy.
\end{proof}

%\begin{remark}
%Making use of admissible function spaces, stronger conditions for characterizing the exponential dichotomy of the DAE \eqref{eq2} have been obtained in \cite{HuyP19b}, where an inhomogeneity function $g(t)$ belongs to less restricted spaces than $\BEA$. There, we also study the robustness of exponential dichotomy under structured-perturbations.%
%\end{remark}
...
\begin{proposition}
Consider the semi-linear DAE \eqref{eq1}. Furthermore, assume that all three functions $\Si^{-1}$, $\tA^{-1}_4$, $\Si^{-1} \tA_2 \tA^{-1}_4$ are bounded. If the function $f = \m{f_1 \\ f_2}$ belongs to the class $(M,\vphi,\rho)$ then the following claims hold true. \\
