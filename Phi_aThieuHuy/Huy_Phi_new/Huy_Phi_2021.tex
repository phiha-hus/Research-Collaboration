...
and the unstable subspace ($\ker(P(t))$). Now let us recall some basic concepts and properties for DAEs, starting with \emph{fundamental solution matrix} as below.
%
\begin{definition}
...
\end{proof}
%
Let $\hA_3(t) := -\tA^{-1}_4(t) \tA_3(t)$, $\hA_1(t) := \Si^{-1}(t) \ \left( \tA_1(t) + \tA_2(t) \tA^{-1}_4(t) \tA_3(t) \right)$, we rewrite system \eqref{semi-explicit system} as 
%
\begin{align}
...
Consider the DAE \eqref{eq1} and the evolution family $(X(t,s))_{t\geq s\geq 0}$ defined by \eqref{eq4}. 
For the function $U$ defined in Lemma \ref{lem5}, let $\hf(t,x) := U(t)f(t,x)$. Then, the solution to \eqref{eq1}, if exists, also satisfies the so-called \emph{mild equation}
	%
	\begin{equation*}
		\m{x_1(t) \\ x_2(t)} = \hX(t,s) \m{x_1(s) \\ x_2(s)} + \int_{s}^{t} \hX(t,\tau) \m{\hf_1( \tau,x_1(\tau),x_2(\tau) ) \\ 0 } d\tau  \ + \m{ 0 \\ \hf_2( t,x_1(t),x_2(t) )},
	\end{equation*}
	for all $t\geq s \geq 0$, where $\hf_1 := \m{\Si^{-1}(t) & 0} \hf$ and $\hf_2 := \m{0 & -\tA^{-1}_4(t)} \hf$.
\end{lemma}
\begin{proof}
...
\begin{proposition}\label{prop1} (\cite{LinM09})
	The DAE \eqref{eq2} has exponential dichotomy if and only if the corresponding underlying ODE \eqref{eq5.1} also has exponential dichotomy, and the matrix function $\hA_3(t)$ is bounded. Moreover, the existence of exponential dichotomy imlies that $\underset{t\geq 0}{\sup \ } \|\Py(t)\| < \infty$. 
\end{proposition}
...
The main result of this section is to prove a characterization of the exponential dichotomy for DAEs. Roughly speaking, under some conditions, the DAE \eqref{eq2} admits exponential dichotomy if and only if the mapping $\cL := E\dfrac{d}{dt} - A$ is surjective on the space $C_b(\r_+)$. 
...
\begin{theorem}\label{thm3.1}
	...
	Furthermore, assume that the matrix-valued functions $\Sigma^{-1}(t)$, $\tA_2(t)$ and $\tA^{-1}_4(t)$ are uniformly bounded on $\r_+$. 
	Then the following assertions hold true. 
	\begin{enumerate}[(i)]
	\item If the DAE \eqref{eq2} admits exponential dichotomy then for any continuous, bounded function $g(t)$ on $[0,\infty)$, there exists a continuous, bounded solution $x(t)$ to the DAE \eqref{eq3.1}. 
