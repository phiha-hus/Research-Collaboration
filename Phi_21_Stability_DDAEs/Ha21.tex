...
\begin{equation}\label{index reduced system}
	\underbrace{\m{I & 0 \\ 0 & \mathbf{0} }}_{=:\tE} \dot{y}(t) = \underbrace{\m{ J & 0 \\ 0 & I}}_{=:\tA_0} y(t) + \sum_{i=1}^{m} \underbrace{\tAi{i}}_{=:\tA_i} y(t-\tau_i).
\end{equation}
%
\end{lem}
\begin{pf} 
...
We, again, notice that all the conditions on the references mentioned above are only valid for impulse-free system. 
In order to apply these results for arbitrarily-high index system, first we transform system \eqref{free system} to the index-reduced form \eqref{index reduced system}.
