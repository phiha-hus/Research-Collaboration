...
\begin{lem}\label{KW lemma}(\citet{Dai89,KunM06}) Provided that the matrix pair $(E,A_0 )$ is regular, then there exist regular matrices $W$, $T\in \R^{n,n}$ such that
%
...
\begin{rem}
In general, the two concepts index and stability are independent. In fact, Examples 5 in \citet{HaN21} has illustrated that there exist systems with arbitrarily high-index (and hence, not impulse-free) which are stable.
\end{rem}
...
\begin{equation}\label{index reduced system}
	\underbrace{\m{I & 0 \\ 0 & 0 }}_{=:\tE} \dot{y}(t) = \underbrace{\m{ J & 0 \\ 0 & I}}_{=:\tA_0} y(t) + \sum_{i=1}^{m} \underbrace{\tAi{i}}_{=:\tA_i} y(t-\tau_i).
\end{equation}
%
\end{lem}
\begin{pf} 
...
Assume that the pair $(E,A_0 )$ is regular with index $\ind(E,A_0 )=\nu$. We want to give a simple check whether the system \eqref{free system} is non-advanced or not. In analoguous to the case of DAEs, see e.g. \citet{BreCP96,KunM06}, we aim to extract the so-called \emph{underlying delay equation} of the form 
%
...
Consider the linear, homogeneous DDAE \eqref{free system}. Furthermore, assume that it is non-advanced. Then system \eqref{free system} has the same spectrum (without counting multiplicity), and also the same stability property as the index-reduced system \eqref{index reduced system}.
\end{lem}
\begin{pf} Firstly, we will show that both systems \eqref{free system} and \eqref{index reduced system} have the same spectrum (without counting multiplicity) as the 
...
\end{align*}
%
Due to Lemma \ref{Neumann inverse}  and \eqref{non-advanced cond.}, we have
%
\begin{align*}
...
%
Now let us consider the stability of \eqref{free system}. Clearly, system \eqref{free system} is stable if and only if system \eqref{eq9} is also stable, and vice versa. Hence, due to Lemma \ref{lem4}ii) we obtain the desired claim.  \hfill $\square$
\end{pf}

Lemma \ref{key lemma} shows that a non-advanced system \eqref{free system} has the same spectrum and the same stability property as the index-reduced system \eqref{index reduced system}, which is impulse-free. Therefore, Proposition \ref{Wim11} applied to system \eqref{index reduced system} leads us to 
...
\end{equation}
%
where $\Si_E$, $\Si_A$ are nonsingular, upper triangular matrices, $N_E$ is a nilpotent, upper triangular matrix.
...
\eqref{free system} and the following impulse-free system
%
...
\subsection{Lyapunov-Krasovskii aproach}           
