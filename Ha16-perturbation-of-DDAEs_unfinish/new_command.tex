...
\def\ml{\mu[L]}
\newcommand{\n}[1]{\| #1 \|}
\newcommand\bign[1]{\left\lVert#1\right\rVert}
...
%\newcommand {\ker}   {\mathop{\rm kernel}\nolimits}
\newcommand {\coker} {\mathop{\rm cokernel}\nolimits}


\def\cej{ \cE(t,\eta_{j-1}) }
\def\cejj{ \cE(\eta_{j},\eta_{j-1})}
\def\zej{ z(\eta_{j-1}) }
\def\xtt{ x(t-\tau(t)) }
\def\xts{ x(s-\tau(s))}
\newcommand{\ntwo}[1]{\left\vert\kern-0.25ex\left\vert\kern-0.25ex\left\vert #1 \right\vert\kern-0.25ex\right\vert\kern-0.25ex\right\vert}
